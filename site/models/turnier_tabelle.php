...
class CLMModelTurnier_Tabelle extends JModelLegacy
{
    ...
    public function _getTurnierPlayers()
    {
        ...
        $turParams = new clm_class_params($this->turnier->params);
        $qualiUp = (int) $turParams->get('qualiUp', 0);
        $qualiUpPoss = $qualiUp + (int) $turParams->get('qualiUpPoss', 0);
        $qualiDown = (int) $turParams->get('qualiDown', 0);
        $qualiDownPoss = $qualiDown + (int) $turParams->get('qualiDownPoss', 0);
    ...
    public function _getTurnierTeams()
    {
        ...
        foreach ($this->players as $key => $value) {
            ...
            if (!isset($a_teams[$value->mtln_nr])) {
                continue;
            }
    ...
    //Sonderranglisten
    public function _getSpecialRankingWhere()
    {
        $where = "";
        if ($this->spRang != 0) {
