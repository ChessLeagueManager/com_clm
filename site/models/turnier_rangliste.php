...
class CLMModelTurnier_Rangliste extends JModel {
		...
		if($this->spRang != 0){
			...
			if($this->spRank->use_rating_filter == 1){
				if($this->spRank->rating_type == 0){
					...
					$where = $where ." AND IF(NATrating > 0, NATrating, FIDEelo) >= '".$this->spRank->rating_higher_than."'"
									." AND IF(NATrating > 0, NATrating, FIDEelo) <= '".$this->spRank->rating_lower_than."'";
				} elseif($this->spRank->rating_type == 4){
					$where = $where ." AND IF(FIDEelo > 0, FIDEelo, NATrating) >= '".$this->spRank->rating_higher_than."'"
									." AND IF(FIDEelo > 0, FIDEelo, NATrating) <= '".$this->spRank->rating_lower_than."'";
				} elseif($this->spRank->rating_type == 5){
					$where = $where ." AND IF(FIDEelo > NATrating, FIDEelo, NATrating) >= '".$this->spRank->rating_higher_than."'"
									." AND IF(FIDEelo > NATrating, FIDEelo, NATrating) <= '".$this->spRank->rating_lower_than."'";
				}
			}
