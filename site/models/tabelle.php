...
class CLMModelTabelle extends JModelLegacy
{
	...
	// ordering für Rangliste -> Ersatz für direkten Vergleich
		...
		$query = " SELECT a.tln_nr as tln_nr,m.name as name, "
			...
			." SUM(case when a.manpunkte = ".$order[0]->man_sieg." then 1 else 0 end) as count_S, "
			." SUM(case when a.manpunkte = ".$order[0]->man_remis." then 1 else 0 end) as count_R, "
			." SUM(case when a.manpunkte = ".$order[0]->man_nieder." then 1 else 0 end) as count_V, "
