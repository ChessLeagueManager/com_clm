...
// Email Erstellung
	$subject = Text::_('REGISTRATION_ONLINE').' - '.$turnier->name;
	$body_daten = Text::_('REGISTRATION_PLAYER').': '.$reg_name."\n";
	if ($reg_vorname != '') $body_daten .=  Text::_('REGISTRATION_VORNAME').': '.$reg_vorname."\n";
	$body_daten .=	Text::_('REGISTRATION_JAHR').': '.$reg_birthYear."\n"
				. Text::_('REGISTRATION_CLUB').': '.$reg_club."\n"
				. Text::_('REGISTRATION_MAIL').': '.$reg_mail."\n";
	if ($reg_dwz != '') $body_daten .=  Text::_('REGISTRATION_DWZ').': '.$reg_dwz."\n";
	if ($reg_elo != '') $body_daten .=  Text::_('REGISTRATION_ELO').': '.$reg_elo."\n";
	if ($reg_FIDEid != '') $body_daten .=  Text::_('REGISTRATION_FIDEID').': '.$reg_FIDEid."\n";
	if ($reg_tel_no != '') $body_daten .=  Text::_('REGISTRATION_TEL_NO').': '.$reg_tel_no."\n";
	if ($reg_account != '') $body_daten .=  Text::_('REGISTRATION_ACCOUNT_'.$typeAccount).': '.$reg_account." \n\n";
	if ($reg_comment != '') $body_daten .=  Text::_('REGISTRATION_COMMENT').': '."\n".$reg_comment."\n";
...
//	$url = URI::base().'components/com_clm/clm/mail_approve.php?parameter='.$randomUid;
	...
	if ($email_TL != "") {
		$body_TL = $turnier->name."\n\n".Text::_('REG_TC_HELLO_1')."\n".Text::_('REG_TC_HELLO_2')."\n".$body_daten;
//		$result = clm_core::$cms->sendMail($from, $fromname, $email_TL, $subject, $body_TL, $htmlMail);
		...
		if ($result[0] !== true) 
			$msg .= '<br>'.Text::_('REG_MAIL_NOT_TO_TL').'<br>'.$result[1];
	}
	// Info-E-mail an den Spieler - info e-mail to player
	if ($reg_mail != "") {
		$body_AM = $turnier->name."\n\n".Text::_('REG_PLAYER_HELLO_1')."\n".Text::_('REG_PLAYER_HELLO_2')."\n".Text::_('REG_PLAYER_HELLO_3')."\n".$body_daten;
		$body_AM .= "\n".Text::_('REG_PLAYER_HELLO_4');
		$body_AM .= "\n".$url;
		$body_AM .= "\n".Text::_('REG_PLAYER_HELLO_5');
//		$result = clm_core::$cms->sendMail($from, $fromname, $reg_mail, $subject, $body_AM, $htmlMail);
		...
		if ($result[0] !== true) 
			$msg .= '<br>'.Text::_('REG_MAIL_NOT_TO_PL').'<br>'.$result[1];
	}
