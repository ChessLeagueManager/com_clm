...
$pdf->SetFont('Times','',$date_font);
	...
	$pdf->Cell(100,$zelle+2,clm_core::$load->utf8decode(Text::_('LEAGUE_STAT_BEST')).' : '.clm_core::$load->utf8decode($liga[0]->name),0,1,'L');
	...
	for ($x=0; $x < ($liga[0]->stamm+1); $x++) { 
		...
		$pdf->Cell(100,$zelle+2,clm_core::$load->utf8decode(Text::_('LEAGUE_STAT_BRETT')).' '.$xtext,0,1,'L');
		$pdf->SetFont('Times','',$font-1);
		$pdf->SetTextColor(0);
		$pdf->SetFillColor(200); 	
		$pdf->Cell($lrand,$zelle,' ',0,0);
		$pdf->Cell(8,$zelle,Text::_('LEAGUE_STAT_BRETT'),1,0,'C',1);
		$pdf->Cell(35,$zelle,Text::_('DWZ_NAME'),1,0,'L',1);
		$pdf->Cell(10,$zelle,Text::_('LEAGUE_STAT_DWZ'),1,0,'R',1);
		$pdf->Cell(45,$zelle,Text::_('LEAGUE_STAT_CLUB'),1,0,'L',1);
		for ($xx=1; $xx < 7; $xx++) {   //max. 6 Spalten
			$en = 0;
			if ($params['btiebr'.$xx] == 1) { $hstring = Text::_('LEAGUE_STAT_PLAYERPOINTS'); $width = 9; }
			elseif ($params['btiebr'.$xx] == 2) { $hstring = Text::_('LEAGUE_STAT_PLAYERGAMES'); $width = 9; }
			elseif ($params['btiebr'.$xx] == 3) { $hstring = Text::_('LEAGUE_STAT_PLAYERLEVEL'); $width = 12; }
			elseif ($params['btiebr'.$xx] == 4) { $hstring = Text::_('LEAGUE_STAT_RATING_PDF'); $en = 1; $width = 13; }
			elseif ($params['btiebr'.$xx] == 5) { $hstring = Text::_('LEAGUE_STAT_PERCENT'); $width = 9; }
			elseif ($params['btiebr'.$xx] == 6) { $hstring = Text::_('LEAGUE_STAT_PLAYERPOINTS'); $width = 11; $en = 3; $ey = 1; }
			elseif ($params['btiebr'.$xx] == 7) { $hstring = Text::_('LEAGUE_STAT_PLAYERGAMES'); $width = 11; $en = 3; $ey = 1; }
			elseif ($params['btiebr'.$xx] == 8) { $hstring = Text::_('LEAGUE_STAT_PERCENT'); $width = 9; $en = 3; $ey = 1; }
			elseif ($params['btiebr'.$xx] == 9) { $hstring = Text::_('LEAGUE_STAT_BNUMBERS_PDF'); $width = 13; }
	...
	$pdf->Cell(10,$zelle-2,clm_core::$load->utf8decode(html_entity_decode('&sup1;',ENT_COMPAT)).Text::_('LEAGUE_RATING_COMMENT_PDF'),0,1,'L');
	if($ex >0) { $pdf->Cell($lrand,$zelle-2,'',0,0,'C');
				 $pdf->Cell(10,$zelle-2,clm_core::$load->utf8decode(html_entity_decode('&sup2;',ENT_COMPAT)).Text::_('LEAGUE_RATING_IMPOSSIBLE_PDF'),0,1,'L'); }
	if($ey >0) { $pdf->Cell($lrand,$zelle-2,'',0,0,'C');
				 $pdf->Cell(10,$zelle-2,clm_core::$load->utf8decode(html_entity_decode('&sup3;',ENT_COMPAT)).Text::_('LEAGUE_WITH_UNCONTESTED_PDF'),0,1,'L'); }

// Ausgabe
$pdf->Output(Text::_('LEAGUE_STAT_BEST').' '.clm_core::$load->utf8decode($liga[0]->name).'.pdf','D');
exit;
?>
