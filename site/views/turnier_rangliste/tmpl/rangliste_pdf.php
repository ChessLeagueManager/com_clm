...
$pdf->SetFont('Times','',14);
	...
	if (isset($this->turnier->spRangName) AND $this->turnier->spRangName > '')
		$pdf->Cell(150,15,clm_core::$load->utf8decode($this->turnier->name).": ".clm_core::$load->utf8decode($this->turnier->spRangName)." ".clm_core::$load->utf8decode(Text::_('TOURNAMENT_RANKING')),0,1,'L');
	else
		$pdf->Cell(150,15,clm_core::$load->utf8decode($this->turnier->name).": ".clm_core::$load->utf8decode(Text::_('TOURNAMENT_RANKING')),0,1,'L');
...
$pdf->SetFillColor(0);
	...
	if ($turParams->get('displayPlayerSnr', 1) == 1) {
		$pdf->Cell($br02,$zelle,Text::_('TOURNAMENT_NUMBERABB'),1,0,'C',1); }
	$pdf->Cell($br03,$zelle,Text::_('TOURNAMENT_PLAYERNAME'),1,0,'L',1);
	$pdf->Cell($br04,$zelle,Text::_('TOURNAMENT_TWZ'),1,0,'C',1);
	for ($rnd=1; $rnd<=$this->turnier->runden; $rnd++) {
		$pdf->Cell($br11,$zelle,$rnd,1,0,'C',1); }
	$pdf->Cell($br91,$zelle,Text::_('TOURNAMENT_POINTS_ABB'),1,0,'C',1); 
	// mgl. Feinwertungen
	for ($f=1; $f<=3; $f++) {
		$fwFieldName = 'tiebr'.$f;
...
// Ausgabe
if (isset($this->turnier->spRangName) AND $this->turnier->spRangName > '')
	$pdf->Output(clm_core::$load->utf8decode(Text::_('TOURNAMENT_RANKING')).' '.clm_core::$load->utf8decode($this->turnier->name).' '.clm_core::$load->utf8decode($this->turnier->spRangName).'.pdf','D');
else
	$pdf->Output(clm_core::$load->utf8decode(Text::_('TOURNAMENT_RANKING')).' '.clm_core::$load->utf8decode($this->turnier->name).'.pdf','D');
exit;
?>
