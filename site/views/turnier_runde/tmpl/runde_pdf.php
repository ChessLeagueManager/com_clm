...
$pdf->SetFont('Times','',14);
	$pdf->Cell(10,15,' ',0,0);
	//$heading = clm_core::$load->utf8decode($this->turnier->name).": ".clm_core::$load->utf8decode(Text::_('TOURNAMENT_ROUND'))." ".$this->round->nr;
	...
	if ($this->round->datum != "0000-00-00" AND $this->round->datum != "1970-01-01" AND $turParams->get('displayRoundDate', 1) == 1) {
		$heading .=  ", ".clm_core::$load->utf8decode(HTMLHelper::_('date',  $this->round->datum, Text::_('DATE_FORMAT_CLM_F'))); 
...
// Ausgabe
$pdf->Output(clm_core::$load->utf8decode(Text::_('TOURNAMENT_ROUND'))." ".$this->round->nr.' '.clm_core::$load->utf8decode($this->turnier->name).'.pdf','D');
exit;
?>
