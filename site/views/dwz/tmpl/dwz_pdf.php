...
<div class="componentheading"><?php echo clm_core::$load->utf8decode(Text::_('CLUB_UNKNOWN')) ?></div>
<?php	} else {
...
$pdf->SetFont('Times','',16);
	...
	if ($countryversion =="de") {
		$pdf->Cell(80,15,clm_core::$load->utf8decode(Text::_('CLUB_RATING')).' '.clm_core::$load->utf8decode($liga[0]->Vereinname),0,1,'L');
	} else {
		$pdf->Cell(80,15,clm_core::$load->utf8decode(Text::_('CLUB_RATING_EN')).' '.clm_core::$load->utf8decode($liga[0]->Vereinname),0,1,'L');
	}
// User und Saison
...
if (!$archive_check) {
	...
	$pdf->Cell(10,10,' ',0,0);
	$pdf->Cell(80,10,clm_core::$load->utf8decode(Text::_('NO_ACCESS')),0,1,'L');
	$pdf->Cell(10,10,' ',0,0);
	$pdf->Cell(80,10,clm_core::$load->utf8decode(Text::_('NOT_REGISTERED')),0,1,'L');
...
$pdf->SetFont('Times','',$font);
	...
	$pdf->Cell(7,$zelle,clm_core::$load->utf8decode(Text::_('CLUB_NR')),1,0,'C');
	if ($countryversion =="de") {
		if ($cuser != -1) {
			$pdf->Cell(16,$zelle,clm_core::$load->utf8decode(Text::_('CLUB_MEMBER')),1,0,'C');
		}
	} else {
		$pdf->Cell(18,$zelle,clm_core::$load->utf8decode(Text::_('CLUB_MEMBER_PKZ')),1,0,'C');
	}
	$pdf->Cell(80,$zelle,clm_core::$load->utf8decode(Text::_('CLUB_MEMBER_NAME')),1,0,'C');
	if ($countryversion =="de") {
		$pdf->Cell(16,$zelle,clm_core::$load->utf8decode(Text::_('CLUB_MEMBER_RATING')),1,0,'C');
	} else {
		$pdf->Cell(16,$zelle,clm_core::$load->utf8decode(Text::_('CLUB_MEMBER_RATING_EN')),1,0,'C');
	}
	$pdf->Cell(16,$zelle,clm_core::$load->utf8decode(Text::_('CLUB_MEMBER_RATINGS')),1,0,'C');
	$pdf->Cell(16,$zelle,clm_core::$load->utf8decode(Text::_('CLUB_MEMBER_ELO')),1,0,'C');
	// Zeilenumbruch
	$pdf->Ln();
...
$pdf->Output(clm_core::$load->utf8decode(Text::_('CLUB_RATING')).' '.clm_core::$load->utf8decode($liga[0]->Vereinname).'.pdf','D');
exit;
}
?>
