...
if($spRang != 0){			//Sonderranglisten
	...
	$heading = $this->turnier->name.": ".Text::_('TOURNAMENT_TABLE');
}
	...
	if ($fixth_ttab =="1") { echo 'class="tableWithFloatingHeader"'; };
		...
		echo '><tr>';
			...
			if ($turParams->get('displayPlayerTitle', 1) == 1) {
				echo '<th class="titel">'.Text::_('TOURNAMENT_TITLE').'</th>';
			}
			echo '<th class="name_float">'.Text::_('TOURNAMENT_PLAYERNAME').'</th>';
			if ($turParams->get('displayPlayerClub', 1) == 1) {
				echo '<th class="verein">'.Text::_('TOURNAMENT_CLUB').'</th>';
			}
			echo '<th class="twz">'.Text::_('TOURNAMENT_TWZ').'</th>';
			echo '<th class="fw_col">'.Text::_('TOURNAMENT_GAMES_ABB').'</th>';
			echo '<th class="fw_col">'.Text::_('TOURNAMENT_POINTS_ABB').'</th>';
			// mgl. Feinwertungen
			for ($f=1; $f<=3; $f++) {
				$fwFieldName = 'tiebr'.$f;
				if ($this->turnier->$fwFieldName > 0 AND $this->turnier->$fwFieldName < 50) {
					echo '<th class="fw_col">'.Text::_('TOURNAMENT_TIEBR_ABB_'.$this->turnier->$fwFieldName).'</th>';
				}
			}
		echo '<tr />';
