...
// Componentheading
$heading =  $this->turnier->name.": ".Text::_('Warteliste');
...
if (!$archive_check) {
    ...
    echo CLMContent::componentheading($heading);
	require_once(JPATH_COMPONENT.DS.'includes'.DS.'submenu_t.php');
	echo CLMContent::clmWarning(Text::_('Warteliste ist leer'));

	
} else {

// PDF-Link
echo CLMContent::createPDFLink('turnier_waitlist', Text::_('Warteliste'), array('turnier' => $this->turnier->id, 'layout' => 'waitlist'));
    echo CLMContent::componentheading($heading);
	require_once(JPATH_COMPONENT.DS.'includes'.DS.'submenu_t.php');
	...
	foreach ($this->players as $key => $value) {
		...
		<tr class="<?php echo $zeilenr; ?>">
			...
			$link->more = array('turnier' => $this->turnier->id, 'snr' => $value->snr, 'Itemid' => $itemid, 'list' => 'wait' );
			$link->makeURL();
