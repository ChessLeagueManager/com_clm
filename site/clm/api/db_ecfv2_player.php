...
function clm_api_db_ecfv2_player($zps = - 1, $mgl_nr = array())
{
    ...
    foreach ($a_out->column_names as $key => $value) {
        ...
        if ($value == 'original_standard') {
            $i_original_standard = $key;
        }
        if ($value == 'club_code') {
            $i_club_code = $key;
        }
    ...
    foreach ($a_out->players as $player) {
        $i++;
        if ($zps != $player[$i_club_code]) {
            continue;
        }
        if ('' == $player[$i_ECF_code]) {
            continue;
        }
        ...
        $fide_no = (int) $player[$i_FIDE_no];
        if (isset($i_ECF_junior)) {
            if ($player[$i_ECF_junior] == true) {
                $junior = 1;
            } else {
                $junior = 0;
            }
        ...
        if ($player[$i_gender] == 'F') {
            $gender = 'W';
        } else {
            $gender = $player[$i_gender];
        }
