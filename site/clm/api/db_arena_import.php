...
function clm_api_db_arena_import($arena_code, $season, $turnier, $group = false, $update = false, $test = false)
{
    ...
    if ($debug > 0) {
        echo "<br><br>datei-code: ".$arena_code;
    }
    ...
    if (!$lines) { // code ist kein Turnier im Arena-Modus
        ...
        if (!$lines) { // code ist auch kein Turnier im CH-Modus
            ...
            if ($debug > 0) {
                echo "<p>Tournament with CH-Mode! / Turnier im Schweizer Sytem!</p>";
            }
            $tur_mode = 'swiss';
        }
    } else {
        if ($debug > 0) {
            echo "<p>Tournament with Arena-Mode! / Turnier im Arena-Modus!</p>";
        }
        $tur_mode = 'arena';
    }
    if ($debug > 1) {
        echo "<br>Turnier-Modus: ".$tur_mode;
    }
    ...
    {
        if ($debug > 0) {
            echo "<br><br>-- uelement --".$uelement;
        } //die();
        $upos = strpos(($contents), $uelement.' ', $ustart);
        if ($debug > 0) {
            echo "<br><br>-- upos --".$upos;
        } //die();
        if ($upos === false) {
            return '';
        }
    ...
    while (($pos = strpos(($contents), $toFind, $start)) !== false) {
        $i++;
        if ($debug > 1) {
            echo '<br>'.$i.' Found '.$toFind.' at position '.$pos."\n";
        }
        $start = $pos;
        $event = pgn_element($contents, 'Event', $start);
        if ($debug > 0) {
            echo "<br><br>-- Event --".$event;
        } // var_dump($event); } //die();
        ...
        if ($debug > 0) {
            echo "<br>-- UTCDate --".$utcdate;
        } // var_dump($utcdate); } //die();
        $utctime = pgn_element($contents, 'UTCTime', $start);
        if ($debug > 0) {
            echo "<br>-- UTCTime --".$utctime;
        } // var_dump($utctime); } //die();
        $whiteelo = pgn_element($contents, 'WhiteElo', $start);
        if ($debug > 0) {
            echo "<br>-- WhiteElo --".$whiteelo;
        } // var_dump($whiteelo); } //die();
        $blackelo = pgn_element($contents, 'BlackElo', $start);
        if ($debug > 0) {
            echo "<br>-- BlackElo --".$blackelo;
        } // var_dump($blackelo); } //die();
    ...
    foreach ($a_spieler as $key => $spieler) {
        ...
        if ($debug > 0) {
            echo "<br>-- spieler --:".($key + 1).' '.$spieler->name.' '.$spieler->elo;
        } // var_dump($n_spieler); }
    }
    if ($debug > 1) {
        echo "<br><br>-- n_spieler --:";
        var_dump($n_spieler);
    }
    if ($debug > 1) {
        echo "<br><br>-- s_spieler --:";
        var_dump($s_spieler);
    }
    ...
    foreach ($a_paar as $key => $paar) {
        ...
        if (!isset($a_runde[$runde])) {
            ...
            if ($dateStart == '1970-01-01') {
                $dateStart = str_replace('.', '-', $paar->utcdate);
            }
    ...
    $anz_runden = $runde;
    if ($a_runde[($runde)]->datum != '0000-00-00') {
        $dateEnd = str_replace('.', '-', $a_runde[($runde)]->datum);
    } else {
        $dateEnd = '1970-01-01';
    }
    if ($debug > 1) {
        echo "<br><br>-- r_spieler --:";
        var_dump($r_spieler);
    }
    if ($debug > 1) {
        echo "<br><br>-- a_paar --:";
    ...
    if ($group) { 	// Mannschaftsturniere
        ...
        if ($debug > 0) {
            echo "<br>-- Teilnehmer --:".$teil;
        } //die();
        if ($debug > 0) {
            echo "<br>-- Runden --:".$runden;
        } //die();
    ...
    if ($debug > 0) {
        echo "<br><br><b>neue Turnier-ID in swt-Tabellen: ".$new_swt_tid."</b>";
    }
    ...
    foreach ($a_runde as $key => $runde) {
        ...
        if ($debug > 0) {
            echo "<br>-- Datum --:".$rdatum;
        } //die();
        if ($debug > 0) {
            echo "<br>-- Startzeit --:".$startzeit;
        } //die();
    ...
    if ($debug > 0) {
        echo "<br><br>-- Einzel-Ergebnisdaten --<br>";
    }
    $runde = 1;
    $brett = 0;
    $paar = 1;
    ...
    foreach ($a_paar as $key => $paar) {
        ...
        if ($debug > 0) {
            echo "<br>Runde: $runde  Brett: $brett  Spieler: $spieler  Gegner: $gegner  Paar-result: $paar->result   Ergebnis: $ergebnis";
        } //var_dump($ergebnis);	}
        if ($gegner > 16000) {
            $gegner = 0;
        }
        if ($debug > 0) {
            echo "<br>Runde: $runde  Brett: $brett  Spieler: $spieler  Gegner: $gegner  Ergebnis: $ergebnis  -- ";
        } //var_dump($ergebnis);	}
    ...
    if ($tur_mode == 'swiss') {
        if ($debug > 0) {
            echo "<br><br>-- Einzel-Ergebnisdaten ohne Partie aus TRF-Datei --<br>";
        }
        if ($debug > 0) {
            echo "<br><br>-- Einzel-Ergebnisdaten Korrekturen aus TRF-Datei --<br>";
        }
        $a_spielerdaten = array();
        $file = 'https://lichess.org/swiss/'.$arena_code.'.trf';
        if ($debug > 0) {
            echo "<br>TRF-Datei: ".$file;
        }
        $lines = file($file);
        if ($debug > 1) {
            echo "<br>lines:";
        ...
        if (!$lines) {
            ...
            foreach ($lines as $line_num => $line) {
                ...
                if ($din == 'XXR') {
                    $rundenzahl = trim(substr($line, 4));
                }
            }
        }
        if ($debug > 0) {
            echo "<br>Rundenzahl: ".$rundenzahl;
        }
        if ($debug > 1) {
            echo "<br>a_spielerdaten:";
            var_dump($a_spielerdaten);
        }
        ...
        foreach ($a_spielerdaten as $line) {
            ...
            for ($i = 0; $i < $rundenzahl; $i++) {
                ...
                if ($gegner > 0) {
                    if ($debug > 1) {
                        echo "<br>a_trf_nr[gegner]:";
                        var_dump($a_trf_nr[$gegner]);
                    }
        ...
        for ($i = 1; $i <= $rundenzahl; $i++) {
            for ($j = 1; $j <= $a_brett[$i]; $j++) {
                ...
                if ($a_paarung[$i][$j]['werg'] == 'U' or $a_paarung[$i][$j]['werg'] == 'H') {
                    ...
                    if ($a_paarung[$i][$j]['werg'] == 'U') {
                        $ergebnis = 5;
                    }
                    if ($a_paarung[$i][$j]['werg'] == 'H') {
                        $ergebnis = 12;
                    }
                    $a_runde[$runde]->count++;
                    $brett = $a_runde[$runde]->count;
                    if ($debug > 0) {
                        echo "<br> Ergebnis ohne Partie: Runde: ".$runde."  Spieler: $spieler  Gegner: $gegner   Ergebnis: $ergebnis  ";
                    }
                    $keyS = '`sid`, `swt_tid`, `dg`, `runde`, `brett`, `tln_nr`, `heim`, `spieler`, `gegner`, `ergebnis`, `pgn`';
                    ...
                    if ($p_result != '--0' and $p_result != '0--') {
                        ...
                        if ($debug > 1) {
                            echo "<br>Runde: ".$runde."  Brett: ".$brett."  Spieler: $spieler  Gegner: $gegner   PErgebnis: $p_result  WErgebnis: $wergebnis  BErgebnis: $bergebnis  ";
                        }
                        if (isset($a_paar_spl[$runde][$spieler][$gegner])
                            ...
                            if ($debug > 1) {
                                echo "<br> ja: Runde: ".$runde."  Brett: ".$brett."  Spieler: $spieler  Gegner: $gegner  ";
                            }
                        } else {
                            if ($debug > 0) {
                                echo "<br> Ergebniskorrektur: Runde: ".$runde."  Brett: ".$brett."  Spieler: $spieler  Gegner: $gegner  Ergebnis: $p_result ";
                            }
