...
function clm_api_db_pgn_template($id, $dg, $round, $type, $group = true)
{
    ...
    if ($type == 1) {
        $file_name .= '_'.clm_core::$load->utf8decode($user_zps);
    }
    ...
    if ($group) {
        foreach ($matches as $einz) {
            if (($einz->zps == $user_zps) or ($einz->gzps == $user_zps) or ($type == 2)) {
                ...
                if (is_null($einz->gelo) or $einz->gelo == 0) {
                    $einz->gelo = '';
                }
                if (is_null($einz->helo) or $einz->helo == 0) {
                    $einz->helo = '';
                }
                if (is_null($einz->gdwz) or $einz->gdwz == 0) {
                    $einz->gdwz = '';
                }
                if (is_null($einz->hdwz) or $einz->hdwz == 0) {
                    $einz->hdwz = '';
                }
                if ($einz->weiss == "0") {
                    ...
                    if ($einz->ergebnis == 2) {
                        ...
                        $gtmarker = "1/2-1/2";
                    } elseif ($einz->ergebnis == 1) {
                        fputs($pdatei, '[Result "0-1"]'.$nl);
                        $gtmarker = "0-1";
                    } elseif ($einz->ergebnis == 0) {
                        fputs($pdatei, '[Result "1-0"]'.$nl);
                        $gtmarker = "1-0";
                    } elseif ($einz->ergebnis == 4) {
                        fputs($pdatei, '[Result "1-0"]'.$nl);
                        $resulthint = "{".clm_core::$load->utf8decode($lang->white)."}";
                        $gtmarker = "1-0";
                    } elseif ($einz->ergebnis == 5) {
                        fputs($pdatei, '[Result "0-1"]'.$nl);
                        $resulthint = "{".clm_core::$load->utf8decode($lang->black)."}";
                        $gtmarker = "0-1";
                    } elseif ($einz->ergebnis == 6) {
                        fputs($pdatei, '[Result "*"]'.$nl);
                        $resulthint = "{".clm_core::$load->utf8decode($lang->nobody)."}";
                        $gtmarker = "*";
                    } else {
                        fputs($pdatei, '[Result "'.$einz->erg_text.'"]'.$nl);
                    }
                } else {
        ...
        foreach ($matches as $value) {
            if (($value->spieler != 0 and $value->gegner != 0) or !is_null($value->ergebnis)) {
                ...
                if ($value->ergebnis == "2") {
                    fputs($pdatei, '[Result "1/2-1/2"]'.$nl);
                    $gtmarker = "1/2-1/2";
                } elseif ($value->ergebnis == "0") {
                    fputs($pdatei, '[Result "0-1"]'.$nl);
                    $gtmarker = "0-1";
                } elseif ($value->ergebnis == "1") {
                    fputs($pdatei, '[Result "1-0"]'.$nl);
                    $gtmarker = "1-0";
                } elseif ($value->ergebnis == "5") {
                    fputs($pdatei, '[Result "1-0"]'.$nl);
                    $resulthint = "{".clm_core::$load->utf8decode($lang->white)."}";
                    $gtmarker = "1-0";
                } elseif ($value->ergebnis == "4") {
                    fputs($pdatei, '[Result "0-1"]'.$nl);
                    $resulthint = "{".clm_core::$load->utf8decode($lang->black)."}";
                    $gtmarker = "0-1";
                } elseif ($value->ergebnis == "6") {
                    fputs($pdatei, '[Result "*"]'.$nl);
                    $resulthint = "{".clm_core::$load->utf8decode($lang->nobody)."}";
                    $gtmarker = "*";
                } else {
                    fputs($pdatei, '[Result "'.$value->ergebnis.'"]'.$nl);
                }
                fputs($pdatei, '[PlyCount "0"]'.$nl);
