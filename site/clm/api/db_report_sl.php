...
*/
function clm_api_db_report_sl($liga, $runde, $dg, $paar, $apaar)
{
    ...
    $apaar = clm_core::$load->make_valid($apaar, 18, '');

    $out["input"]["liga"] = $liga;
    $out["input"]["runde"] = $runde;
    $out["input"]["dg"] = $dg;
    $out["input"]["paar"] = $paar;
    $out["input"]["apaar"] = $apaar;
    //echo "<br>apaar".$apaar;
    //die();
    ...
    */
    ...
    if ($someData[0]->rang != "0") {
        $Heim = $Heim.",r.rang ,r.man_nr as rmnr";
    }
    $Heim = $Heim
    ...
    */
    ...
    if ($someData[0]->rang != "0") {
        $Gast = $Gast.",r.rang,r.man_nr as rmnr ";
    }
    $Gast = $Gast
    ." FROM #__clm_meldeliste_spieler as a ";
    if ($countryversion == "de") {
    ...
    if ($finish[($runde + (($dg - 1) * $out["liga"][0]->runden) - 1)]->deadlinetime != '00:00:00') {
        $mdt .= $finish[($runde + (($dg - 1) * $out["liga"][0]->runden) - 1)]->deadlinetime;
    } else {
        $mdt .= '00:00:00';
    }
    ...
    // Alle Pararungen der Runde
    $apaarModel = " SELECT a.*,g.id as gid, g.name as gname, g.tln_nr as gtln,"
        ." h.id as hid, h.name as hname, h.tln_nr as htln, b.brettpunkte as gbrettpunkte, "
        ." g.zps as gzps, g.sg_zps as gsgzps, g.mf as gast_mf, "
        ." h.zps as hzps, h.sg_zps as hsgzps, h.mf as heim_mf "
        ." FROM #__clm_rnd_man as a "
        ." LEFT JOIN #__clm_mannschaften AS g ON g.tln_nr = a.gegner "
        ." LEFT JOIN #__clm_mannschaften AS h ON h.tln_nr = a.tln_nr "
        ." LEFT JOIN #__clm_rnd_man AS b ON b.lid = a.lid AND b.dg = a.dg AND b.runde = a.runde AND b.paar = a.paar AND b.heim = 0 "
        ." WHERE a.runde = ".$runde
        ." AND a.lid = ".$liga
        ." AND a.heim = 1 "
        ." AND a.dg = ".$dg
//		." AND a.paar = ".$paar
        ...
        ." ORDER BY a.paar "
    ;
    $out["apaar"] = clm_core::$db->loadObjectList($apaarModel);
    //echo "<br>apaar<br>"; var_dump($out["apaar"]);

    $allresult	= "SELECT paar, SUM(CASE WHEN (ergebnis <> 7 AND ergebnis > -1) THEN 1 ELSE 0 END) AS serg "
        ." FROM #__clm_rnd_spl "
        ." WHERE lid = $liga AND runde = $runde "
//		." AND paar = $paar AND dg = $dg AND heim = 1 "
        ." AND dg = $dg AND heim = 1 "
        ." GROUP BY paar "
    ;

    $out["allresult"] = clm_core::$db->loadObjectList($allresult);
    //die();
