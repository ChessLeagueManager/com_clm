...
// Output: ELO-Durchschnitt
function clm_api_db_elo_average($lid,$runde=0,$dg=1) {
	...
	if ($runde == 0) {
		...
		foreach ($mliste as $mliste1) {
			...
			if ($params_dwz_date == '0000-00-00' OR $params_dwz_date == '1970-01-01') {
				if (isset($mliste1->elo) AND is_numeric($mliste1->elo) AND $mliste1->elo > 0) {
					$a_sum_elo[$mliste1->tln_nr] += $mliste1->elo; 
					$a_count_player[$mliste1->tln_nr]++;
				} elseif ($params_pseudo_dwz > 0) {
					$a_sum_elo[$mliste1->tln_nr] += $params_pseudo_dwz; 
					$a_count_player[$mliste1->tln_nr]++;
				}
			} else {
				if (isset($mliste1->FIDEelo) AND is_numeric($mliste1->FIDEelo) AND $mliste1->FIDEelo > 0) {
					$a_sum_elo[$mliste1->tln_nr] += $mliste1->FIDEelo; 
					$a_count_player[$mliste1->tln_nr]++;
				} elseif ($params_pseudo_dwz > 0) {
					$a_sum_elo[$mliste1->tln_nr] += $params_pseudo_dwz; 
					$a_count_player[$mliste1->tln_nr]++;
		...
		if ($params_pseudo_dwz > 0) {
			for ($s=0; $s <= $l[0]->teil; $s++) { // alle Startnummern durchgehen
				if ($a_count_player[$s] > 0 AND $a_count_player[$s] < $l[0]->stamm ) {
					$a_sum_elo[$s] += ($l[0]->stamm - $a_count_player[$s]) * $params_pseudo_dwz;
	...
	for ($s=0; $s <= $l[0]->teil; $s++) { // alle Startnummern durchgehen
		if ($a_count_player[$s] > 0) {
			$a_average_elo[$s] = round($a_sum_elo[$s]/$a_count_player[$s]);
			$a_average_elo_p[$s] = (string) $a_average_elo[$s];
		} 
	}
