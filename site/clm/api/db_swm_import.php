...
function clm_api_db_swm_import($file, $season, $turnier, $group = false, $update = false, $test = false)
{
    ...
    if ($debug > 1) {
        echo "<br>tournament2: ";
        var_dump($tournament2);
    }
    $tournament["out"][1] = $tournament2["out"][1]; // Anzahl Runden
    $arunden = (int) $tournament["out"][1][0]; // Anzahl Runden
    if ($group) {
        $tournament["out"][2] = $tournament2["out"][2]; // Anzahl Mannschaften
        $teil = (int) $tournament["out"][2][0]; // Anzahl Mannschaften
        $tournament["out"][3] = $tournament2["out"][3]; // Anzahl Bretter
        $stamm = (int) $tournament["out"][3][0]; // Anzahl Bretter
    }
    ...
    if ($tournament['out'][65][0] > '') {
        $name = $tournament['out'][65][0];
    } // Turniername
    else {
        $name = $tournament['out'][12][0];
    }
    ...
    if ($tournament_fk[0] > $tournament["out"][213][0]) {
        $tournament["out"][213][0] = $tournament_fk[0];
    }
    ...
    if ($group) {
        ...
        if (strpos($file, '.TUR') > 0 or strpos($file, '.tur') > 0) {
            $typ = '2';
        } // Einzel-Rundenturnier	 .TUR
    ...
    if ($debug > 0) {
        echo "<br><br><b>neue Turnier-ID (swm): ".$new_swt_tid."</b>";
    }
    ...
    if ($debug > 1) {
        echo "<br>paarprorunde: ";
        var_dump($paarprorunde);
    }
    if ($debug > 1) {
        echo "<br>tableprorunde: ";
        var_dump($tableprorunde);
    }
    ...
    for ($i = 0; $i < $tournament["out"][4][0]; $i++) {
        ...
        if ($debug > 0) {
            echo "<br>2060 ".$lang->t2060.': '.$tab_record['out'][2060][0];
        }
        ...
        */
        ...
        if ($tab_record['out'][2002][0] == '') {
            $tab_record['out'][2002][0] = $tab_record['out'][2045][0];
        }
        ...
        if ($debug > 2) {
            echo "<br>paramuseAsTWZ: $paramuseAsTWZ  twz: $twz  tab_player: ";
            var_dump($tab_record);
        }
        ...
        $dwz = (int) $tab_record['out'][2004][0];
        $elo = (int) $tab_record['out'][2003][0];
        $fideid = (int) $tab_record['out'][2033][0];
        ...
        if ($group) { 	// Mannschaftsturniere
            $manid = (int) $tab_record['out'][2061][0];
            $brett = (int) $tab_record['out'][2062][0];
    ...
    for ($i = 0; $i < 5000; $i++) {
        ...
        $slength += $tab_record['length'];
        if ($debug > 1) {
            echo "<br>tab_record: $i ";
            var_dump($tab_record);
        }
        if ($debug > 0) {
            echo " ( Runde: $runde  Brett: $brett ) ";
        }
        ...
        if ($debug > 1) {
            echo "<br>runde: $runde  brett: $brett  ergebnis: $ergebnis  -- ";
            var_dump($ergebnis);
        }
        if (($spieler == 0 or $gegner == 0) and $ergebnis == 7) {
            continue;
        }
        if ($ergebnis == 99) {
            continue;
        ...
        if ($group) { 	// Mannschaftsturniere
            ...
            if ($epaar > $paarprorunde[$runde]) {
                $runde++;
                if (!isset($tableprorunde[$runde]) or $tableprorunde[$runde] == 0) {
                    break;
                }
                $epaar = 1;
                $paar = 1;
            ...
            if ($debug > 0) {
                echo " ( Runde: $runde  Paar: $paar  Brett: $brett  Teilnehmer: $tln_nr  Punkte: $punkte) ";
            }
            ...
            if ($debug > 0) {
                echo " ( Runde: $runde  Paar: $paar  Brett: $brett  Teilnehmer: $gtln_nr  Punkte: $punkte) ";
            }
            if (!is_null($ergebnis)) {
    ...
    if ($group) { 	// Mannschaftsturniere
        ...
        for ($i = 0; $i < $tournament["out"][2][0]; $i++) {
            ...
            if ($debug > 1) {
                echo "<br>3060 ".$lang->t3060.': '.$tab_record['out'][3060][0];
            }
            $slength += $tab_record['length'];

            $name = $tab_record['out'][3040][0];
            $land = (int) $tab_record['out'][3043][0];
            $tlnnr = (int) $tab_record['out'][3060][0];
            $mannr = (int) $tab_record['out'][3060][0];
    ...
    if ($group) { 	// Mannschaftsturniere
        $a_man_paar = array();
        if ($debug > 0) {
            echo "<br><br>-- Mannschafts-Paarungen --";
        }
        $slength = 0;
        $runde = 1;
        $table = 0;
        ...
        if ($debug > 0) {
            echo "<br>Summe Einzelergebnisse  $sum_m_punkte ";
        }
        if ($debug > 0) {
            if ($sum_m_punkte == 0) {
                echo "<br>Einzelergebnisse wurden nicht eingegeben! ";
            }
        }
        //for ($i = 0; $i < $tournament["out"][1][0] * $paar_zahl; $i++) {
        for ($i = 0; $i < 5000; $i++) {
            ...
            if ($debug > 0) {
                echo " ( Runde: $runde  Tisch: $table  Spieler: $spieler  Gegner: $gegner  Brettpunkte Heim $brettpunkte_heim  Brettpunkte Gast $brettpunkte_gast) ";
            }
            if ($debug > 1) {
                echo "<br>m_punkte: ";
                var_dump($m_punkte);
            ...
            if ($gegner < 16000) {			// normaler Kampf
                if ($sum_m_punkte == 0) {
                    $h_punkte = $brettpunkte_heim;
                } else {
                    $h_punkte = $m_punkte[$runde][$spieler];
                }
                if ($sum_m_punkte == 0) {
                    $g_punkte = $brettpunkte_gast;
                } else {
                    $g_punkte = $m_punkte[$runde][$gegner];
                }
                if ($h_punkte > $g_punkte) {
                    $man_punkte = 2;
                } elseif ($h_punkte < $g_punkte) {
                    $man_punkte = 0;
                } else {
                    $man_punkte = 1;
                }
            ...
            if ($debug > 1) {
                echo "<br><br>runde: $runde  brett: $brett  ergebnis: $ergebnis  -- ";
                var_dump($ergebnis);
            }
...
function zzparse_interpret($binary, $part, $group, $start = 0, $end = false, $debug = 0)
{
    ...
    if ($debug > 1) {
        echo "<br>part: $part   start: $start   end: $end";
    }
    $lang = clm_core::$lang->swm_import;
    if ($debug > 3) {
        echo "<br>binary: $binary   <br>ende";
    }
    if ($end) {
    ...
    $data['out'] = array();
    if ($group) {
        $structure = zzparse_structure_t($part);
    }  // s-single = Einzelturnier
    else {
        $structure = zzparse_structure_s($part);
    }		  // t-team = Mannschaftswettbewerb
    if ($debug > 3) {
        echo "<br>structure: ";
        var_dump($structure);
    } //die();
    ...
    foreach ($structure as $line) {
        ...
        switch ($line[0]) {
            ...
            case 'vch':
                ...
                */            $istart++;
                ...
                if ($debug > 2) {
                    echo "<br>substring: $substring";
                }
                $istart += (2 * $length);
                if ($debug > 2) {
                    echo "   vch-istart: $istart";
                }
                //die();
                $len = strlen($substring);
                if ($debug > 2) {
                    echo "<br>len: $len   substring: $substring";
                }
                $output = '';
                for ($a = 0; $a < $len; $a++) {
                    //if ($debug > 2) { echo "<br>a: $a   byte: ".bin2hex(substr($substring,$a,1)); }
                    if (bin2hex(substr($substring, $a, 1)) != '00') {
                        $output .= bin2hex(substr($substring, $a, 1));
                    }
                }
                $len = strlen($output);
                if ($debug > 2) {
                    echo "<br>len: $len   output: $output  line2: ".$line[2];
                }
                $substring = hexToStr($output);
                if (($line[2] === 'startzeit') and (strlen($substring) < 6)) {
                    $substring .= ':00';
                }
                $data['out'][$line[1]][0] = addslashes(clm_core::$load->utf8encode($substring));
                $tnr = 't'.$line[1];
                if ($debug > 1) {
                    echo "<br>".$tnr.'/'.$lang->{$tnr}.'/'.$substring.'/'.$data['out'][$line[1]][0];
                }
                break;
            case 'ign':
                if ($debug > 2) {
                    echo "<br>1ign-istart: $istart";
                }
                // Content to ignore
                // first parameter (begin) with length
                $length = $line[1];
                if ($debug > 2) {
                    echo "<br>ign length: $length";
                }
                if ($length < 0 or $length > 256) {
                    $lenght = 0;
                    if ($debug > 2) {
                        echo "<br>length-error  byte:$istart";
                    }
                }
                $substring = substr($binary, $istart, $length);
                if ($debug > 2) {
                    echo "<br>content: $substring";
                }
                $istart += +$length;
                if ($debug > 2) {
                    echo "<br>ign-istart: $istart";
                }
                break;
            ...
            case 'inb':
                ...
                if ($debug > 2) {
                    echo "<br>1substring: $substring  d1: $d1  d2: $d2";
                }
                $istart++;
                $istart++;
            ...
            case 'ind':
                ...
                if ($debug > 2) {
                    echo "<br>datum: ohne datum  i: $i  d1: $d1  d2: $d2  d3: $d3  d4: $d4  lt: $lt ";
                }
            ...
            case 'boo':
                ...
                switch ($substring) {
                    case 'FF': $data['out'][$line['content']] = 1;
                        break;
                    case '00': $data['out'][$line['content']] = 0;
                        break;
                    default: $data['out'][$line['content']] = null;
                        break;
                }
                break;
            ...
            case 'sel':
                ...
                if (preg_match('/^sel\:\d+/', $line['type'])) {
                    $area = str_replace('sel:', '', $line['type']);
                }
                $area .= '-selection';
        ...
        if ($line[0] != 'ign') {
            $data['out'][$line[1]][1] = $line;
        }
        if ($debug > 1 and $line[0] != 'ign') {
            echo "<br>content:";
            var_dump($data['out'][$line[1]]);
            echo "<br>total:";
            var_dump($data['out']);
        }
        if ($debug > 1 and $line[0] != 'ign') {
...
function transcode_tiebr($group, $tournament, $line_nr, $debug)
{
    ...
    */
    ...
    if ($debug > 1) {
        echo "<br>1tiebr linenr:";
        var_dump($line_nr);
    }
    if ($debug > 1) {
        echo "<br>1tiebr line:";
        var_dump($tournament["out"][$line_nr]);
    }
    if ($debug > 1) {
        echo "<br>1tiebr line+4:";
        var_dump($tournament["out"][$line_nr + 4]);
    }
    ...
    if ($debug > 1) {
        echo "<br>streich_schwach: $streich_schwach   streich_stark: $streich_stark";
    }
    //die();
    ...
    if ($group) {
        if ($line[0] == 1) {		// Brettpunkte
            ...
            if ($streich_stark == 0 and $streich_schwach == 0) {
                $line[0] = 6;
            } else {
                $line[0] = 0;
            }
            ...
            if ($streich_stark == 0 and $streich_schwach == 0) {
                $line[0] = 1;
            } elseif ($streich_stark == 0 and $streich_schwach == 1) {
                $line[0] = 11;
            } // mit 1 Streichwert
            elseif ($streich_stark == 1 and $streich_schwach == 1) {
                $line[0] = 7;
            }	// mittlere Buchholz
            else {
                $line[0] = 0;
    ...
    if ($debug > 0) {
        echo "<br>feinwertung  swm: $swm  ->  clm: ".$line[0];
    }
    //die();
...
function transcode_ergebnis($ergebnis, $heim, $gegner)
{
    ...
    */
    if ($gegner > 16000 and $ergebnis == 1) {
        $ergebnis = 11;
    }
    if ($gegner > 16000 and $ergebnis == 2) {
        $ergebnis = 12;
    }
    if ($gegner > 16000 and $ergebnis == 3) {
        $ergebnis = 13;
    }
    if ($heim == 1) {
