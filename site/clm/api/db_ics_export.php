...
function clm_api_db_ics_export($file_name, $itermine)
{
    ...
    foreach ($itermine as $termin) {
        if (!isset($termin['DSTART'])) {
            continue;
        }
        if (!is_numeric($termin['DSTART'])) {
            continue;
        }
        if (!isset($termin['SUMMARY'])) {
            continue;
        }
        $termine[] = $termin;
    }
    ...
    if (!file_exists('components'.DS.'com_clm'.DS.'ics'.DS)) {
        mkdir('components'.DS.'com_clm'.DS.'ics'.DS);
    }
