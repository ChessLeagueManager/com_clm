...
function clm_api_db_player_check($sid, $zps, $mglnr)
{
    ...
    if ($sid == -1 or strlen($zps) != 5 or $mglnr == -1) {
        return array(false, 'Kein Check möglich');
    }
    ...
    if (!is_null($result) and count($result) > 0) {
        return array(false, 'Ergebnisse in Teamwettbewerben liegen vor');
    }
    ...
    $result = clm_core::$db->loadObjectList($query);
    if (!is_null($result) and count($result) > 0) {
        return array(false, 'Spieler wird in Rangfolge geführt');
    }
    ...
    $result = clm_core::$db->loadObjectList($query);
    if (!is_null($result) and count($result) > 0) {
        return array(false, 'Spieler wird in Meldeliste geführt');
    }
    ...
    $result = clm_core::$db->loadObjectList($query);
    if (!is_null($result) and count($result) > 0) {
        return array(false, 'Benutzer ist mit Mitgliedsnummer angelegt');
    }
    ...
    $result = clm_core::$db->loadObjectList($query);
    if (!is_null($result) and count($result) > 0) {
        return array(false, 'Spieler ist Teilnehmer in Einzelturnier');
    }
