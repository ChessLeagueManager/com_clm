...
//			." ORDER BY a.tln_nr "
        ...
        if (($runden_modus == 4) or ($runden_modus == 5)) {
            return array(true, "e_ligaCalculateRankingSuccess");
        }
        ...
        if ($p_runde != 0) {
            ...
            foreach ($matchData as $key => $value) {
                if ($value->ergebnis < 3 and ((($value->dg - 1) * $runden) + $value->runde) > $maxround) {
                    ...
                    foreach ($matchDataSnr as $key => $valuesnr) {
                        ...
                        if (($valuesnr->dg < $value->dg) or ($valuesnr->dg == $value->dg and $valuesnr->runde < $value->runde)) {
                            if ($valuesnr->ergebnis == 1) {
                                $PlayerPunkteKOR += $man_sieg;
                            } // Sieg
                            elseif ($valuesnr->ergebnis == 2) {
                                $PlayerPunkteKOR += $man_remis;
                            } // remis
            ...
            if (in_array(7, $arrayFW) or in_array(8, $arrayFW) or in_array(17, $arrayFW) or in_array(18, $arrayFW)) { // beliebige Buchholz als TieBreaker gewünscht?
                if ($value->ergebnis < 3 or $params['optionTiebreakersFideCorrect'] == 0) {
                    ...
                    foreach ($matchDataSnr as $key => $valuesnr) {
                        ...
                        if (($valuesnr->dg < $value->dg) or ($valuesnr->dg == $value->dg and $valuesnr->runde < $value->runde)) {
                            if ($valuesnr->ergebnis == 1) {
                                $PlayerPunkteKOR += $mbrett_sieg;
                            } // Sieg
                            elseif ($valuesnr->ergebnis == 2) {
                                $PlayerPunkteKOR += $mbrett_remis;
                            } // remis
                            elseif ($valuesnr->ergebnis == 5) {
                                $PlayerPunkteKOR += $mbrett_sieg;
                            } // Sieg kampflos
        ...
        if ((in_array(7, $arrayFW)) or (in_array(8, $arrayFW)) or (in_array(18, $arrayFW))) { // normale Buchholz als TieBreaker gewünscht?
            for ($s = 1; $s <= $team[0]->teil; $s++) { // alle Startnummern durchgehen
                ...
                if (!isset($array_PlayerBuchOppBP[$s])) {
                    ...
                    $array_PlayerBuchBP[$s] = $array_PlayerBuchOppBP[$s][0];
                } else {
                    $array_PlayerBuchBP[$s] = array_sum($array_PlayerBuchOppBP[$s]);
                }
            }
        }
        if (in_array(17, $arrayFW)) { // Buchholz mit Streichresultat
            for ($s = 1; $s <= $team[0]->teil; $s++) { // alle Startnummern durchgehen
                ...
                if (!isset($array_PlayerBuchOppBP[$s])) {
                    ...
                    $array_PlayerBuch1StBP[$s] = array_sum($array_PlayerBuchOppBP[$s]);
                }
            }
        }
        ...
        if ($team[0]->tiebr1 == 25 or $team[0]->tiebr2 == 25 or $team[0]->tiebr3 == 25) {
            ...
            foreach ($players as $xvalue) {
                ...
                foreach ($players as $yvalue) {
                    if ($xvalue->tln_nr == $yvalue->tln_nr) {
                        continue;
                    }
                    ...
                    if (($p_runde == 0 and
                        ...
                        ;
                        if ($p_runde != 0) {
                            $query .= " AND runde < ".($p_runde + 1);
                        }
                        $matchesdirect = clm_core::$db->loadObjectList($query);
                        $zdirect = count($matchesdirect);
                        foreach ($matchesdirect as $mdvalue) {
                            if ($mdvalue->ergebnis == '0') {
                                $sum_erg += $team[0]->man_nieder + $team[0]->man_antritt;
                            } elseif ($mdvalue->ergebnis == '1') {
                                $sum_erg += $team[0]->man_sieg + $team[0]->man_antritt;
                            } elseif ($mdvalue->ergebnis == '2') {
                                $sum_erg += $team[0]->man_remis + $team[0]->man_antritt;
                            } elseif ($mdvalue->ergebnis == '5') {
                                $sum_erg += $team[0]->man_sieg + $team[0]->man_antritt;
                            }
                        }
                    }
                ...
                if ($id_dv == 1) {
                    ...
                    if ($p_runde == 0) {
                        ...
                        if ($team[0]->tiebr1 == 25) {
                            $query .= " SET z_sumtiebr1 = ".$sum_erg;
                        } elseif ($team[0]->tiebr2 == 25) {
                            $query .= " SET z_sumtiebr2 = ".$sum_erg;
                        } else {
                            $query .= " SET z_sumtiebr3 = ".$sum_erg;
                        }
                    }
                    $query .= " WHERE liga = ".$id
                    ...
                    if ($p_runde == 0) {
                        ...
                        $query = "UPDATE #__clm_mannschaften";
                        if ($team[0]->tiebr1 == 25) {
                            $query .= " SET z_sumtiebr1 = NULL";
                        } elseif ($team[0]->tiebr2 == 25) {
                            $query .= " SET z_sumtiebr2 = NULL";
                        } else {
                            $query .= " SET z_sumtiebr3 = NULL";
                        }
                    }
                    $query .= " WHERE liga = ".$id
                        . " AND tln_nr = ".$xvalue->tln_nr
        ...
        if ($liga_mt == 0) {
            if ($p_runde == 0) {
                ...
                if ($b_wertung == 0 and $order == 1) {
                    $query .= ", ordering ASC";
                }
                if ($b_wertung == 3 and $order == 1) {
                    $query .= ", sumtiebr1 DESC, ordering ASC";
                }
                if ($b_wertung == 3 and $order == 0) {
                    $query .= ", sumtiebr1 DESC";
                }
                if ($b_wertung == 4 and $order == 1) {
                    $query .= ", ordering ASC, sumtiebr1 DESC";
                }
                if ($b_wertung == 4 and $order == 0) {
                    $query .= ", sumtiebr1 DESC";
                }
            } else {
                ...
                if ($b_wertung == 3 and $order == 1) {
                    $query .= ", z_sumtiebr1 DESC, ordering ASC";
                }
                if ($b_wertung == 3 and $order == 0) {
                    $query .= ", z_sumtiebr1 DESC";
                }
                if ($b_wertung == 4 and $order == 1) {
                    $query .= ", ordering ASC, z_sumtiebr1 DESC";
                }
                if ($b_wertung == 4 and $order == 0) {
                    $query .= ", z_sumtiebr1 DESC";
                }
                    ...
                    if ($p_runde == 0) {
                        if ($liga_mt == 0) {
                            if ($value->summanpunkte == $value0->summanpunkte and $value->sumbrettpunkte == $value0->sumbrettpunkte and $value->sumtiebr1 == $value0->sumtiebr1) {
                                if ($order == 0) {
                                    $s_rankingPos = 0;
                                } elseif ($value->order == $value0->order) {
                                    $s_rankingPos = 0;
                                }
                            }
                        } else {
