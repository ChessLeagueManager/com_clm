...
function clm_view_termine_ics($out)
{
    ...
    foreach ($db_termine as $termin1) {
        ...
        if ($termin1->startdate == "1970-01-01" or $termin1->startdate == "0000-00-00") {
            continue;
        }
        ...
        if ($termin1->starttime == "00:00:00" or $termin1->starttime == "24:00:00") {
            $starttime .= '0000';
        } else {
            $starttime = substr($termin1->starttime, 0, 2).substr($termin1->starttime, 3, 2);
        }
        ...
        if ($termin1->enddate == "1970-01-01" or $termin1->enddate == "0000-00-00") {
            $termin1->enddate = '0000000000';
        }
        $enddate = substr($termin1->enddate, 0, 4).substr($termin1->enddate, 5, 2).substr($termin1->enddate, 8, 2);
        if (!is_numeric($enddate)) {
            $enddate = '00000000';
        }
        if ($termin1->endtime == "00:00:00" or $termin1->endtime == "24:00:00") {
            $endtime = '0000';
        } else {
            $endtime = substr($termin1->endtime, 0, 2).substr($termin1->endtime, 3, 2);
        }
        ...
        if ($termin1->address > '') {
            $event['LOCATION'] = $termin1->address;
        }
        ...
        */        if ($termin1->host > '0') {
            $event['CLM-HOST'] = $termin1->host;
        } //$termin1->host;
        if (substr($termin1->event_link, 0, 4) == 'http') {
            $event['ATTACH'] = $termin1->event_link;
        }
