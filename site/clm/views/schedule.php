...
function clm_view_schedule($out)
{
    ...
    $lang1 = clm_core::$lang->ics_export;
    ...
    $clm_schedule_message = clm_core::$load->request_string('fnr', '');
    unset($_GET["fnr"]);
    ?>
...
<?php
        if ($clm_schedule_message != '') {
            echo '<br><h5>'.$lang1->$clm_schedule_message.'</h5><br>';
        }
    ?>
	<div class="clmbox">
<?php
        if (isset($paar[0])) {
            echo clm_core::$load->create_link($lang->title_club_details, 'verein', array('saison' => $paar[0]->sid, 'zps' => $club[0]->zps))
...
<?php	echo '<br />';
    ...
    echo '</h4></th><th>';
    if (isset($paar[0])) {
        echo clm_core::$load->create_link_pdf('schedule', $lang->pdf, array('layout' => 'schedule', 'season' => $paar[0]->sid, 'liga' => $paar[0]->lid, 'club' => $club[0]->zps));
    }
    echo "</th><th>";
    if (isset($paar[0])) {
        echo clm_core::$load->create_link_xls('schedule', $lang->csv, array('layout' => 'schedule', 'season' => $paar[0]->sid, 'liga' => $paar[0]->lid, 'club' => $club[0]->zps));
    }
    echo "</th><th>";
    if (isset($paar[0])) {
        echo clm_core::$load->create_link_ics('schedule', $lang->ics, array('layout' => 'schedule', 'season' => $paar[0]->sid, 'liga' => $paar[0]->lid, 'club' => $club[0]->zps));
    }
    echo "</th></tr></table>";
    ?>
...
<div class="element date"><?php
                                if ($paar1->rdate > "1970-01-01") {
                                    echo clm_core::$cms->showDate($paar1->rdate, "d M Y");
                                    if ($paar1->rtime != "00:00:00" and $paar1->rtime != "24:00:00") {
                                        echo "<br>".substr($paar1->rtime, 0, 5);
                                    }
                                }
        ?></div>
<div class="element league">
    <a href="index.php?option=com_clm&view=paarungsliste&saison=<?php echo $paar1->sid; ?>&liga=<?php echo $paar1->lid; ?><?php if ($itemid <> 0) {
        echo "&Itemid=".$itemid;
    } ?>">
...
<div class="element home">
    <a href="index.php?option=com_clm&view=mannschaft&saison=<?php echo $paar1->sid; ?>&liga=<?php echo $paar1->lid; ?>&tlnr=<?php echo $paar1->htln; ?><?php if ($itemid <> 0) {
        echo "&Itemid=".$itemid;
    } ?>">
...
<div class="element guest">
    <a href="index.php?option=com_clm&view=mannschaft&saison=<?php echo $paar1->sid; ?>&liga=<?php echo $paar1->lid; ?>&tlnr=<?php echo $paar1->gtln; ?><?php if ($itemid <> 0) {
        echo "&Itemid=".$itemid;
    } ?>">
