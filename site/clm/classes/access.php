...
class clm_class_access
{
     ...
     */
    public function api($api, $input = array(), $onlyPublished = true)
    {
        return clm_class_access::apiWithId($this->typeId, $api, $input, $onlyPublished);
    }
    public function access($accesspoint, $onlyPublished = true)
    {
    ...
    public static function compareWithId($id1, $id2, $onlyPublished = true)
    {
        ...
        foreach ($accesspoints as $key => $accesspoint) {
            ...
            if (is_int($key)) {
                $str_ap = strval($key);
            }    // verhindert die Deprecated-Meldung; gesamte Funktion ist eventuell inkorrekt!
            else {
                $str_ap = $key;
            }
