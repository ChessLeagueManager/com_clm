...
class CLMViewSWTLigaerg extends JViewLegacy {
	...
	function display ($tpl = null) {
		...
		ToolBarHelper::title( Text::_('TITLE_SWT_LEAGUE_ERG') ,'clm_headmenu_manager.png' );
		...
		if ($bool_mt_runden && ($runde + 1 < $anz_runden || $dgang + 1 < $anz_durchgaenge)) {
			ToolBarHelper::custom('next','forward.png','forward_f2.png', Text::_('SWT_LEAGUE_NEXT_ROUND'), false);
		}
		else {
			ToolBarHelper::custom('finish','save.png','save_f2.png', Text::_('SWT_LEAGUE_FINISH'), false);
		}
		...
		for ($p = 1; $p <= $anz_paarungen; $p++) {
			...
			for ($b = 1; $b <= $swt_db_data['anz_bretter']; $b++) {
				...
				$hsplist = array ();
				$hsplist[] = HTMLHelper::_('select.option', '0', Text::_( 'SWT_LEAGUE_PLAYER_SELECT' ), 'id', 'text');
				$hsplist = array_merge( $hsplist, $db_splist[$heim] );
				$hblist = HTMLHelper::_('select.genericlist', $hsplist, 'hbrett_'.$p.'_'.$b, 'class="inputbox" size="1"', 'id', 'text', $hspieler);

				if (isset($swt_data[$p]['gbrett_'.$b])) $gspieler = $swt_data[$p]['gbrett_'.$b];
				else $gspieler = 0;
				$gsplist = array ();
				$gsplist[] = HTMLHelper::_('select.option', '0', Text::_( 'SWT_LEAGUE_PLAYER_SELECT' ), 'id', 'text');
				$gsplist = array_merge( $gsplist, $db_splist[$gast] );
				$ablist = HTMLHelper::_('select.genericlist', $gsplist, 'gbrett_'.$p.'_'.$b, 'class="inputbox" size="1"', 'id', 'text', $gspieler);
...
/*				echo "<br/> erg_str: " . $swt_data[$p]['ergstr_'.$b]; //DBG
				...
				$erglist = array ();
				$erglist[] = HTMLHelper::_('select.option', 'NULL', Text::_( 'SWT_LEAGUE_RESULT_SELECT' ), 'ergid', 'text');
				$erglist = array_merge( $erglist, $db_erglist );
				$results = HTMLHelper::_('select.genericlist', $erglist, 'erg_'.$p.'_'.$b, 'class="inputbox" size="1"', 'ergid', 'text', $ergebnis);

				if (isset($swt_data[$p]['ergk_'.$b])) $ergebnisk = $swt_data[$p]['ergk_'.$b]; else $ergebnisk = 0;
				$erglistk = array ();
				$erglistk[] = HTMLHelper::_('select.option', 'NULL', Text::_( 'SWT_LEAGUE_RESULT_SELECT' ), 'ergid', 'text');
				$erglistk = array_merge( $erglistk, $db_erglistk );
				$resultsk = HTMLHelper::_('select.genericlist', $erglistk, 'ergk_'.$p.'_'.$b, 'class="inputbox" size="1"', 'ergid', 'text', $ergebnisk);

				$tablerow = '<tr>'
						  . '<td nowrap="nowrap">'
						  . '<label for="brett_'.$p.'_'.$b.'">' . Text::_( 'SWT_LEAGUE_BOARD_NR' ).' '.$b.'</label>'
						  . '</td>'
						  . '<td>' . $hblist . '</td>'
						  . '<td>' . $ablist . '</td>'
						  . '<td>' . $results . '</td>'
