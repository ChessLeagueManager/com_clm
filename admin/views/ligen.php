...
class CLMViewLigen
{
	public static function setLigaToolbar($new, $sid)
	{
		...
		ToolBarHelper::title( Text::_( 'LEAGUE_BUTTON_7' ).': [ '. $text.' ]', 'clm_headmenu_liga.png' );
  ...
  <fieldset class="adminform">
   <legend><?php echo Text::_( 'LEAGUE_DATA' ); ?></legend>
	...
	if (count($parentArray) > 0)  { // nur, wenn Kategorien existieren
		...
		$catidAlltime = HTMLHelper::_('select.genericlist', $parentlist, 'catidAlltime', 'class="inputbox" size="1" style="max-width: 250px;"', 'id', 'name', intval($row->catidAlltime));
		$catidEdition = HTMLHelper::_('select.genericlist', $parentlist, 'catidEdition', 'class="inputbox" size="1" style="max-width: 250px;"', 'id', 'name', intval($row->catidEdition));
	}
	...
	?>
		...
		<tr>
			...
			<td colspan="5" class="paramlist_value">
				...
				echo HTMLHelper::_('select.genericlist', $optionlist, 'params[addCatToName]', 'class="inputbox"', 'id', 'name', (isset($row->params['addCatToName']) ? $row->params['addCatToName'] : "0"));
				?>
			</td>
		</tr>
	<?php
	...
	<input class="inputbox" type="text" name="teil" id="teil" size="4" maxlength="4" value="<?php echo $row->teil; ?>" />
        ...
        <td width="40" class="key" title="<?php echo Text::_( 'LEAGUE_DEADLINE_ROSTER_HINT' ); ?>">
...
<!--		<select name="runden_modus" id="runden_modus" class="js-example-basic-single" value="<?php echo $row->runden_modus; ?>" size="1"> -->
		...
		<!--<option>- wählen -</option>-->
		<option value="1" <?php if ($row->runden_modus == 1) {echo 'selected="selected"';} ?>><?php echo Text::_( 'LEAGUE_PAIRING_MODE_2' );?></option>
		<option value="2" <?php if ($row->runden_modus == 2) {echo 'selected="selected"';} ?>><?php echo Text::_( 'LEAGUE_PAIRING_MODE_3' );?></option>
		</select>
	</td>
	<td nowrap="nowrap">
	...
	<tr>
		...
		<td colspan="2" class="paramlist_value">
			...
			if ($countryversion == "de") {
				...
				$options[2] = Text::_('OPTION_AUTODWZ_9');
			}
			$optionlist = array();
			foreach ($options as $key => $val) {
				$optionlist[]	= HTMLHelper::_('select.option', $key, $val, 'id', 'name' );
...
//				echo HTMLHelper::_('select.genericlist', $optionlist, 'params[autoDWZ]', 'class="js-example-basic-single"', 'id', 'name', "2");
				echo HTMLHelper::_('select.genericlist', $optionlist, 'params[autoDWZ]', 'class="'.$field_search.'"', 'id', 'name', "2");
			?>
...
<?php } ?>
  ...
  <fieldset class="adminform">
   <legend><?php echo Text::_( 'REMARKS' ); ?></legend>
	...
	<?php if ($new) { ?>
		<tr><font color=red><b>Hinweis:</b></font> Manchmal ist es sinnvoll bzw. nötig, eine Punktspielliga als Mannschaftsturnier anzulegen. Siehe auch<b><a href="https://clm4.de/faq/item/190-punktspielligen-als-liga-oder-als-mannschaftsturnier"><?php echo Text::_(' hier'); ?></a></b></tr>
		<p>&nbsp;&nbsp;</p>
	<?php } ?>
