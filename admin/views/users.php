...
#
					...
					<th class="title">
						<?php echo HTMLHelper::_('grid.sort',   'USER', 'name', @$lists['order_Dir'], @$lists['order'] ); ?>
					</th>
					<th width="10%">
						<?php echo HTMLHelper::_('grid.sort',   'USER_FUNCTION', 'd.name', @$lists['order_Dir'], @$lists['order'] ); ?>
					</th>
					<th width="22%">
						<?php echo HTMLHelper::_('grid.sort',   'VEREIN', 'b.Vereinname', @$lists['order_Dir'], @$lists['order'] ); ?>
					</th>
					<th width="11%">
						<?php echo HTMLHelper::_('grid.sort',   'SAISON', 'c.name', @$lists['order_Dir'], @$lists['order']); ?>
					</th>
					<th width="3%">
						<?php echo HTMLHelper::_('grid.sort',   'USER_ACTIVE', 'u.lastvisitDate', @$lists['order_Dir'], @$lists['order'] ); ?>
					</th>

					<th width="3%">
						<?php echo HTMLHelper::_('grid.sort',   'USER_MAIL', 'a.aktive', @$lists['order_Dir'], @$lists['order'] ); ?>
					</th>
			...
			for ($i=0, $n=count( $rows ); $i < $n; $i++) {
				...
				//$row = $value;
				$row->load( $rows[$i]->id );
				$link 		= Route::_( 'index.php?option=com_clm&section=users&task=edit&id='. $row->id );
...
//				$published 	= HTMLHelper::_('grid.published', $row, $i );
				...
				<tr class="<?php echo 'row'. $k; ?>">
					...
					<td>
								<span class="editlinktip hasTip" title="<?php echo Text::_( 'USER_EDIT' );?>::<?php echo $row->name; ?>">
							<a href="<?php echo $link; ?>">
								<?php echo $row->name; ?></a></span>
					</td>

					<td align="center">
						<?php if ($rows[$i]->kind == "CLM") echo Text::_('ACCESSGROUP_NAME_'.$rows[$i]->usertype); else echo $rows[$i]->funktion;?>
					</td>
...
public static function setUserToolbar()
	{
		...
		// Menubilder laden
		clm_core::$load->load_css("icons_images");
		ToolBarHelper::title(  Text::_( 'USER' ).': [ '. $text.' ]', 'clm_headmenu_benutzer.png' );
		ToolBarHelper::save();
		ToolBarHelper::apply();
...
public static function user( &$row,$lists, $option )
	...
	$email_independent = $config->email_independent;
		...
		<legend><?php echo Text::_( 'USER_DETAILS' ); ?></legend>

		<table class="admintable">
		<tr>
			<td class="key" width="20%" nowrap="nowrap">
			<label for="name"><?php echo Text::_( 'USER_NAME' ).' : '; ?></label>
			</td>
			<td>
			<input class="inputbox" type="text" name="name" id="name" size="30" maxlength="60" value="<?php echo $row->name; ?>" /><?php echo Text::_( 'USER_EXAMPLE_NAME' );?>
			</td>
		</tr>

		<tr>
			<td class="key" width="20%" nowrap="nowrap">
			<label for="username"><?php echo Text::_( 'USER' ).' : '; ?></label>
			</td>
			<td>
			<input class="inputbox" type="text" name="username" id="username" size="30" maxlength="60" value="<?php echo $row->username; ?>" /><?php echo Text::_( 'USER_EXAMPLE_USERNAME' );?>
			</td>
		</tr>
		<?php if ($email_independent == 0 ) { ?>
		<tr>
			<td class="key" width="20%" nowrap="nowrap">
			<label for="email"><?php echo Text::_( 'USER_MAIL' ).' : '; ?></label>
			</td>
			<td>
			<input class="inputbox" type="text" name="email" id="email" size="30" maxlength="60" value="<?php echo $row->email; ?>" /><?php echo Text::_( 'USER_EXAMPLE_MAIL' );?>
			</td>
		</tr>
		<?php }
		...
		<tr>
			<td class="key" width="20%" nowrap="nowrap">
			<label for="email"><?php echo Text::_( 'USER_MAIL_CLM' ).' : '; ?></label>
			</td>
			<td>
			<input class="inputbox" type="text" name="email" id="email" size="30" maxlength="60" value="<?php echo $row->email; ?>" /><?php echo Text::_( 'USER_EXAMPLE_MAIL' );?>
			</td>
		...
		<tr>
			<td class="key" width="20%" nowrap="nowrap">
			<label for="jmail"><?php echo Text::_( 'USER_MAIL_JOOMLA' ).' : '; ?></label>
			</td>
			<td>
			<input class="inputbox" type="text" name="jmail" id="jmail" size="30" maxlength="60" value="<?php echo $row->jmail; ?>" /><?php echo Text::_( 'USER_EXAMPLE_MAIL' );?>
			</td>
		</tr>
		<?php } ?>
		<tr>
			<td class="key" width="20%" nowrap="nowrap">
			<label for="tel_fest"><?php echo Text::_( 'USER_TELEFON' ).' : '; ?></label>
			</td>
			<td>
			<input class="inputbox" type="text" name="tel_fest" id="tel_fest" size="30" maxlength="60" value="<?php echo $row->tel_fest; ?>" /><?php echo Text::_( 'USER_EXAMPLE_PHONE' );?>
			</td>
		</tr>
		<tr>
			<td class="key" width="20%" nowrap="nowrap">
			<label for="tel_mobil"><?php echo Text::_( 'USER_MOBILE' ).' : '; ?></label>
			</td>
			<td>
			<input class="inputbox" type="text" name="tel_mobil" id="tel_mobil" size="30" maxlength="60" value="<?php echo $row->tel_mobil; ?>" /><?php echo Text::_( 'USER_EXAMPLE_MOBILE' );?>
			</td>
		</tr>
		<tr>
			<td class="key" nowrap="nowrap">
			<label for="usertype"><?php echo Text::_( 'USER_FUNCTION' ).' : '; ?></label>
			</td>
			<td>
		...
		<tr>
			<td class="key" width="20%" nowrap="nowrap">
 			<label for="fideid"><?php echo Text::_( 'USER_FIDEID' ).' : '; ?></label>
 			</td>
 			<td>
 			<input class="inputbox" type="text" name="fideid" id="fideid" size="30" maxlength="9" value="<?php echo $row->fideid; ?>" title="<?php echo Text::_( 'USER_FIDEID_HINT' );?>" /><?php echo Text::_( 'USER_EXAMPLE_FIDEID' );?>
 			</td>
 		</tr>
		<tr>
			<td class="key" nowrap="nowrap"><label for="verein"><?php echo Text::_( 'VEREIN' ).' : '; ?></label>
			</td>
			<td>
		...
		<tr>
			<td class="key" width="20%" nowrap="nowrap">
 			<label for="name"><?php echo Text::_( 'USER_MGNR' ).' : '; ?></label>
 			</td>
 			<td>
 			<input class="inputbox" type="text" name="mglnr" id="mglnr" size="30" maxlength="6" value="<?php echo $row->mglnr; ?>" /><?php echo Text::_( 'USER_EXAMPLE_MGNR' );?>
 			</td>
 		</tr>

		<tr>
			<td class="key" width="20%" nowrap="nowrap">
 			<label for="name"><?php if ($countryversion =="de") echo Text::_( 'USER_PKZ' ).' : '; else echo Text::_( 'USER_PKZ_EN' ).' : '; ?></label>
 			</td>
 			<td>
 			<input class="inputbox" type="text" name="PKZ" id="PKZ" size="30" maxlength="9" value="<?php echo $row->PKZ; ?>" /><?php echo Text::_( 'USER_EXAMPLE_PKZ' );?>
 			</td>
 		</tr>
	<?php if ($conf_user_member == 1) { ?>
		<tr>
			<td class="key" nowrap="nowrap"><label for="org_exc"><?php echo Text::_( 'OPTION_ORG_EXC' ).' : '; ?></label>
			</td>
			<td><fieldset class="radio">
			<?php echo HTMLHelper::_('select.booleanlist', 'org_exc', 'class="inputbox"', $row->org_exc); ?>
			</fieldset></td>
		</tr>
	<?php } ?>
		<tr>
...
<br>
  <fieldset class="adminform">
	<table class="adminlist">
	<legend><?php echo Text::_( 'USER_LINE01' ); ?></legend>
	<?php echo Text::_( 'USER_LINE02' ); ?>
	<br><?php echo Text::_( 'USER_LINE03' ); ?>.
	<br><br>
	<tr>
	<td width="100%" valign="top">
		<?php echo $lists['jid']; ?>
