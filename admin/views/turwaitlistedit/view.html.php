...
class CLMViewTurWaitlistEdit extends JViewLegacy {
	...
	function display($tpl = NULL) {
		...
		ToolBarHelper::title( $model->turnierData->name.", ".'Warteliste'.": ".$model->playerData->name, 'clm_turnier.png'  );
		...
		if (($row->tl == clm_core::$access->getJid() AND $clmAccess->access('BE_tournament_edit_detail') == 2) OR $clmAccess->access('BE_tournament_edit_detail') === true) {
			...
			ToolBarHelper::custom('move_to', 'forward.png', 'forward.png', Text::_('REGISTRATION_MOVE_TO'),false);
		}
