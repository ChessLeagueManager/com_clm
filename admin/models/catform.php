...
class CLMModelCatForm extends JModelLegacy {
	...
	function _getData() {
		...
		// Instanz der Tabelle
		$this->category = Table::getInstance( 'categories', 'TableCLM');
...
//		$this->form['parent'] = HTMLHelper::_('select.genericlist', $parentlist, 'parentid', 'class="js-example-basic-single" size="1"', 'id', 'name', intval($this->category->parentid));
		$this->form['parent'] = HTMLHelper::_('select.genericlist', $parentlist, 'parentid', 'class="'.$field_search.'" size="1"', 'id', 'name', intval($this->category->parentid));
