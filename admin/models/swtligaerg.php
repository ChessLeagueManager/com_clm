...
{
    public $debug_ausgaben = -1; // -1 or 1 or 3 or 5
    ...
    public function getSpielerliste()
    {
        ...
        set_time_limit(30);
        $swt_id = clm_escape(clm_core::$load->request_int('swt_id', 0));
        ...
        for ($m = 1; $m <= $anz_mannschaften; $m++) {
            ...
            $sql = ' SELECT m.id, m.tln_nr, m.swt_id '
                    . ' FROM #__clm_swt_mannschaften as m'
                    . ' WHERE m.swt_id = '.$swt_id.' AND m.tln_nr = '.$m;
            $atmp = clm_core::$db->loadObjectList($sql);
            $sql = ' SELECT s.man_id, s.snr, s.mgl_nr as mgl_nr, s.zps as zps, d.Mgl_Nr, d.ZPS, d.Spielername as name,'
                    ...
                    . ' ON s.sid = d.sid AND s.zps = d.ZPS AND s.mgl_nr = d.Mgl_Nr'
                    . ' WHERE s.swt_id = '.$swt_id.' AND s.man_id = '.$atmp[0]->id
                    . ' ORDER BY s.snr ASC';
            ...
            if (!isset($this->_splist) or is_null($this->_splist)) {
                $this->_splist = array();
            } // ab J 4.2 nötig, um notice zu vermeiden
            $this->_splist[$m] = $tmp;
    ...
    public function findSpieler($id)
    {
    ...
    public function getErgebnisliste()
    {
    ...
    public function findPlayerID($name)
    {
    ...
    public function store()
    {
        ...
        $runde	= 1 + clm_core::$load->request_int('runde', 0);
        $dgang	= 1 + clm_core::$load->request_int('dgang', 0);
        ...
        for ($p = 1; $p <= $anz_paarungen; $p++) {
            ...
            if ($noBoardResults == '0') {
                for ($b = 1; $b <= $anz_bretter; $b++) {
                    ...
                    // Einzelergebnisse
                    $hbrett = clm_core::$load->request_int('hbrett_'.$p.'_'.$b);
                    $gbrett = clm_core::$load->request_int('gbrett_'.$p.'_'.$b);

                    $hfarbe		= clm_core::$load->request_string('hfarbe_'.$p.'_'.$b);
                    $hweiss		= ($hfarbe == 'w');
                    ...
                    $gfarbe		= clm_core::$load->request_string('gfarbe_'.$p.'_'.$b);
                    $gweiss		= ($gfarbe == 'w');
                    ...
                    $ergebnis	= clm_core::$load->request_int('erg_'.$p.'_'.$b, null);
                    $kampflos	= ($ergebnis == 4 || $ergebnis == 5 || $ergebnis == 6) * 2; // 0 (nicht kampflos) oder 2 (kampflos)


                    if ($ergebnis == 0) {
                        $hpunkte = $nieder + $antritt;
                        $gpunkte = $sieg + $antritt;
                    } elseif ($ergebnis == 1) {
                        $hpunkte = $sieg + $antritt;
                        $gpunkte = $nieder + $antritt;
                    } elseif ($ergebnis == 2) {
                        $hpunkte = $remis + $antritt;
                        $gpunkte = $remis + $antritt;
                    } elseif ($ergebnis == 3) {
                        $hpunkte = $nieder + $antritt;
                        $gpunkte = $nieder + $antritt;
                    } elseif ($ergebnis == 4) {
                    ...
                    $ergebnisk	= clm_core::$load->request_int('ergk_'.$p.'_'.$b, null);
                    $dwz_edit   = 0;
                    if (isset($ergebnisk) and $ergebnisk > 0) {
                        if ($ergebnisk == 4) {
                            $hmpunkte = $nieder;
                            $gmpunkte = $sieg + $antritt;
                        } // kampflos
                        elseif ($ergebnisk == 5) {
                            $hmpunkte = $sieg + $antritt;
                            $gmpunkte = $nieder;
                        } // kampflos
                        elseif ($ergebnisk == 6) {
                            $hmpunkte = $nieder;
                    ...
                    if (!is_null($ergebnis) and ($ergebnis != 7)) {
                        ...
                        if ($dwz_edit > 0) {
                            $sp_hvalues .= ', "'.$dwz_edit.'", "'.$dwz_editor.'"';
                        }
                        ...
                        if ($dwz_edit > 0) {
                            $sp_gvalues .= ', "'.$dwz_edit.'", "'.$dwz_editor.'"';
                        }
                        $query = ' INSERT IGNORE INTO #__clm_swt_rnd_spl'
                                . ' ( ' . $sp_fields . ' ) '
                                . ' VALUES ( ' . $sp_hvalues . ' ), ( ' . $sp_gvalues . ' ); ';

                        if ($noOrgReference == 1 and $swt_data[$p]['heim_kampflos'] == 2 and $swt_data[$p]['gast_kampflos'] == 2) {
                        } else {
                            //$db->setQuery ($query);
            ...
            $m_hvalues = '"'.$sid.'", "'.$swt_id.'", "'.$runde.'", "'.$p.'", "'.$dgang.'", '
                        . '"1", "'.$htln_nr.'", "'.$gtln_nr.'", ';
            if (is_null($hergebnis)) {
                $m_hvalues .= 'NULL, ';
            } else {
                $m_hvalues .= '"'.$hergebnis.'", ';
            }
            if (is_null($hkampflos)) {
                $m_hvalues .= 'NULL, ';
            } else {
                $m_hvalues .= '"'.$hkampflos.'", ';
            }
            if (is_null($hbrettpunkte)) {
                $m_hvalues .= 'NULL, ';
            } else {
                $m_hvalues .= '"'.$hbrettpunkte.'", ';
            }
            if (is_null($hmanpunkte)) {
                $m_hvalues .= 'NULL, ';
            } else {
                $m_hvalues .= '"'.$hmanpunkte.'", ';
            }
            $m_hvalues .= ' "1"';

            $m_gvalues = '"'.$sid.'", "'.$swt_id.'", "'.$runde.'", "'.$p.'", "'.$dgang.'", '
                        . '"0", "'.$gtln_nr.'", "'.$htln_nr.'", ';
            if (is_null($gergebnis)) {
                $m_gvalues .= 'NULL, ';
            } else {
                $m_gvalues .= '"'.$gergebnis.'", ';
            }
            if (is_null($gkampflos)) {
                $m_gvalues .= 'NULL, ';
            } else {
                $m_gvalues .= '"'.$gkampflos.'", ';
            }
            if (is_null($gbrettpunkte)) {
                $m_gvalues .= 'NULL, ';
            } else {
                $m_gvalues .= '"'.$gbrettpunkte.'", ';
            }
            if (is_null($hmanpunkte)) {
                $m_gvalues .= 'NULL, ';
            } else {
                $m_gvalues .= '"'.$gmanpunkte.'", ';
            }
            $m_gvalues .= ' "1"';
    ...
    public function getDataSWT($during_store = 0)
    {
    ...
    public function _getDataSWTman()
    {
    ...
    public function _getDataSWTspl($during_store = 0)
    {
...
//				echo "mannschaft: $mannschaft"; //DBG
        ...
        if ($this->debug_ausgaben > -1 && $during_store == 0) {
            ...
            for ($p = 1; $p <= $anz_paarungen; $p++) {
                ...
                if ($noBoardResults == '0') {
                    for ($b = 1; $b <=  $anz_bretter; $b++) {
                        if ($noOrgReference == 1 and $swt_man[$p]['heim_kampflos'] == 2 and $swt_man[$p]['gast_kampflos'] == 2) {
                            break;
                        }
    ...
    public function getDataSWTdb()
    {
        ...
        $swt_id = clm_core::$load->request_string('swt_id', '');
