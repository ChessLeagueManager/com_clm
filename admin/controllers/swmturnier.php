...
class CLMControllerSWMTurnier extends JControllerLegacy
{
	...
	function update() {		
		...
		$result = clm_core::$api->db_swm_import($swm,false,true);
	...
	function add() {		
		...
		$result = clm_core::$api->db_swm_import($path.$swm,$sid,false,false,false);
//echo "<br>result:"; var_dump($result); //die();
	...
	function test() {		
		...
		$path = JPATH_COMPONENT . DIRECTORY_SEPARATOR . "swt" . DIRECTORY_SEPARATOR;
		$result = clm_core::$api->db_swm_import($path.$swm,$sid,false,false,true);
