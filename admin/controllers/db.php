...
class CLMControllerDB extends JController
{
	/**
	 * Constructor
	 */
...
function update_clm()
	...
	if($version =="0"){
		...
		$dat3	= substr($lv, -3,1);
		$dat4	= substr($dat, -2,1);
		// Beispiel : LV = 705 --> dat = 05; dat2 = 5; dat3 = 7; dat4 = 0 !! 
	// Landesverband wie NDS / NRW etc.
	if($dat == "00"){
	...
	$og = (substr($lv, 0, 1)).'F999';
		...
		//
		// Sonderfälle sind alle Verbände mit mehr als 9 Unterverbänden 
		// da dann hexadezimal gezählt wird
		//
	...
	// Unterverband bei größeren Landesverbänden
	...
	$sql = " SELECT COUNT(ZPS) as id FROM dwz_spieler ";
	if($version =="0" OR $version =="1"){
		$sql = $sql." WHERE ZPS >= '$ug'"
		." AND ZPS <= '$og'";
	}
	if($version =="2"){
		$sql = $sql." WHERE ZPS IN ($zps_engl)";
	}
	...
	// Daten erster Teil -> wegen Speicherüberlauf !!
	$sql = "SELECT * FROM dwz_spieler ";
	if($version =="0" OR $version =="1"){
		$sql = $sql." WHERE ZPS >= '$ug'"
		." AND ZPS <= '$og'";
	}
	if($version =="2"){
		...
		$sql = $sql." ORDER BY ZPS ASC, Mgl_Nr ASC "
		." LIMIT 0,".((int)($count/2))
		;
	$db->setQuery( $sql);
	$zps_data=$db->loadObjectList();

	for ($x = 0; $x < ((int)($count/2)); $x++) {
		$zps 		= $zps_data[$x]->ZPS;
		$mgl 		= $zps_data[$x]->Mgl_Nr;
		$status 	= $zps_data[$x]->Status;
		$name 		= $zps_data[$x]->Spielername;
		$name_g 	= $zps_data[$x]->Spielername_G;
		$mw 		= $zps_data[$x]->Geschlecht;
		if ($mw != 'W') $mw = 'M'; 
		$berechtigung 	= $zps_data[$x]->Spielberechtigung;
		$geb_jahr 	= $zps_data[$x]->Geburtsjahr;
		$auswertung 	= $zps_data[$x]->Letzte_Auswertung;
		$dwz 		= $zps_data[$x]->DWZ;
		$dwz_index 	= $zps_data[$x]->DWZ_Index;
		$elo 		= $zps_data[$x]->FIDE_Elo;
		$titel 		= $zps_data[$x]->FIDE_Titel;
		$fide_id 	= $zps_data[$x]->FIDE_ID;
		$land 		= $zps_data[$x]->FIDE_Land;
	if (($status != "P") OR ($incl_p == "1")) {
	$sql = " SELECT id FROM #__clm_dwz_spieler "
		." WHERE ZPS = '$zps'"
		." AND Mgl_Nr = ".$mgl
		." AND sid = ".$sid
		;
	$db->setQuery( $sql);
	$exist_id=$db->loadObjectList();
	//$exist = $exist_id[0]->id;

	if (!isset($exist_id[0])) {
		if (!$dwz) {
		$sql = " INSERT INTO #__clm_dwz_spieler "
			." ( `sid`,`PKZ`, `ZPS`, `Mgl_Nr` ,`Status` ,`Spielername` ,`Spielername_G` "
			." ,`Geschlecht`, `Spielberechtigung`,`Geburtsjahr`, `Letzte_Auswertung`, "
			." `FIDE_Elo`, `FIDE_Titel`, `FIDE_ID`, `FIDE_Land`) "
			//." VALUES ( '$sid', '','$zps', '$mgl','$status', '$name', '$name_g', "
			." VALUES ( '$sid', '','$zps', '$mgl','$status', "
			.' "'.$name.'", "'.$name_g.'", '
			." '$mw', '$berechtigung', '$geb_jahr', '$auswertung', "
			." '$elo', '$titel','$fide_id','$land')";
			}
		else {
		$sql = " INSERT INTO #__clm_dwz_spieler "
			." (  `sid`, `PKZ`, `ZPS`, `Mgl_Nr` ,`Status` ,`Spielername` ,`Spielername_G` "
			." ,`Geschlecht`, `Spielberechtigung`,`Geburtsjahr`, `Letzte_Auswertung`, `DWZ`,"
			." `DWZ_Index`, `FIDE_Elo`, `FIDE_Titel`, `FIDE_ID`, `FIDE_Land`) "
			//." VALUES ( '$sid', '','$zps', '$mgl','$status', '$name', '$name_g', "
			." VALUES ( '$sid', '','$zps', '$mgl','$status', "
			.' "'.$name.'", "'.$name_g.'", '
			." '$mw', '$berechtigung', '$geb_jahr', '$auswertung', '$dwz',"
			." '$dwz_index','$elo', '$titel','$fide_id','$land')";
			}
	//$id++;
		}
	else {
		if (!$dwz) {
		$sql = " UPDATE #__clm_dwz_spieler "
			//." SET Status = '$status', Spielername = '$name' , Spielername_G = '$name_g' ,"
			." SET Status = '$status', "
			.' Spielername = "'.$name.'" , Spielername_G = "'.$name_g.'" ,'
			." Geschlecht = '$mw', Spielberechtigung = '$berechtigung', "
			." Geburtsjahr = '$geb_jahr', Letzte_Auswertung = '$auswertung',"
			." FIDE_Elo = '$elo', FIDE_Titel = '$titel', "
			." FIDE_ID = '$fide_id', FIDE_Land = '$land' "
			." WHERE ZPS = '$zps' AND Mgl_Nr = $mgl AND sid = $sid ";
			}
		else {
		$sql = " UPDATE #__clm_dwz_spieler "
			//." SET Status = '$status', Spielername = '$name' , Spielername_G = '$name_g' ,"
			...
			.' Spielername = "'.$name.'" , Spielername_G = "'.$name_g.'" ,'
			." Geschlecht = '$mw', Spielberechtigung = '$berechtigung', "
			." Geburtsjahr = '$geb_jahr', Letzte_Auswertung = '$auswertung', DWZ = '$dwz',"
			." DWZ_Index = '$dwz_index', FIDE_Elo = '$elo', FIDE_Titel = '$titel', "
			." FIDE_ID = '$fide_id', FIDE_Land = '$land' "
			." WHERE ZPS = '$zps' AND Mgl_Nr = $mgl AND sid = $sid ";
			}
		}
	...
	// Array löschen um Speicherplatz freizugeben
	unset($zps_data);

	
	// Daten zweiter Teil -> wegen Speicherüberlauf !!
	$sql = "SELECT * FROM dwz_spieler ";
	if($version =="0" OR $version =="1"){
		$sql = $sql." WHERE ZPS >= '$ug'"
		." AND ZPS <= '$og'";
	...
	if($version =="0" OR $version =="1"){
	if($dat == "00"){
		$sql = "SELECT * FROM dwz_vereine "
			." WHERE LV = '$dat3'"
			;
		} else {
		$sql = " SELECT * FROM dwz_vereine "
			." WHERE ZPS BETWEEN '$ug' AND '$og'"
			;
	}}
	if($version =="2"){
		$sql = " SELECT * FROM dwz_vereine WHERE LV ='".$lv."'";
	}
	
	$db->setQuery( $sql);
	$verein_data=$db->loadObjectList();

	foreach ($verein_data as $verein_data ) {

	$zps 		= $verein_data->ZPS;
	$lv 		= $verein_data->LV;
	$verband 	= $verein_data->Verband;
	$name 		= $verein_data->Vereinname;

	$sql = " SELECT ZPS,LV,Verband,Vereinname FROM #__clm_dwz_vereine "
		." WHERE ZPS = '$zps'"
		." AND sid = ".$sid
		;
	$db->setQuery( $sql);
	$exist_zps	= $db->loadObjectList();
	$verein		= $exist_zps[0]->ZPS;

	if (!$verein) {
	$sql = " INSERT INTO #__clm_dwz_vereine "
		." (  `sid`,`ZPS`, `LV`, `Verband`, `Vereinname` ) "
		." VALUES ( '$sid', '$zps', '$lv','$verband', ";
	if (strpos($name,"'") === false) { $sql .= " '$name')"; }
	else { $sql .= ' "'.$name.'")'; }
	}
	else {
	$sql = " UPDATE #__clm_dwz_vereine "
		." SET ZPS = '$zps', ";
	if (strpos($name,"'") === false) $sql .= " Vereinname = '$name'";
	else $sql .= ' Vereinname = "'.$name.'"';
	$sql .= " WHERE ZPS = '$zps' AND sid = $sid ";
	}
	$db->setQuery($sql);
	$db->query();
	if (mysql_errno() != 0) {
	...
	// Log schreiben
	$clmLog = new CLMLog();
	$clmLog->aktion = "CLM DWZ Update";
	$clmLog->write();
	
	$msg = JText::_('DB_MSG_UPDATE_SUCCESS');
	//$mainframe->redirect( 'index.php?option='. $option.'&section='.$section , $msg);
	$this->setRedirect('index.php?option=com_clm&view=db',$msg); 
	}
...
function delete()
	...
	$filesDir 	= 'components'.DIRECTORY_SEPARATOR.$option.DIRECTORY_SEPARATOR.'upload';
	...
	if ( $datei AND $datei != "all") { JFile::delete( $filesDir.DIRECTORY_SEPARATOR.$datei ); }
	if ($datei =="all") {
		$datei_del = CLMControllerDB::files();
		for ($x=0; $x< count($datei_del); $x++ ) {
			JFile::delete( $filesDir.DIRECTORY_SEPARATOR.$datei_del[$x] );
		}
		}
	if ( $export AND $export != "all") { JFile::delete( $filesDir.DIRECTORY_SEPARATOR.$export ); }
	if ($export =="all") {
		$export_files = CLMControllerDB::export_files();
		for ($x=0; $x< count($export_files); $x++ ) {
			JFile::delete( $filesDir.DIRECTORY_SEPARATOR.$export_files[$x] );
		}
		}
...
function upload_jfile()
	...
	// erlaubte Dateitypen
	$allowed =array('text/plain', 'text/x-sql', 'application/octet-stream', 'text/csv');
	...
	$destDir = JPath::clean(JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.$option.DIRECTORY_SEPARATOR.'upload');
	$dest = JPath::clean($destDir.DIRECTORY_SEPARATOR.$file['name']);
...
function files ()
	...
	$filesDir	= 'components'.DIRECTORY_SEPARATOR.$option.DIRECTORY_SEPARATOR.'upload';
	$files		= JFolder::files( $filesDir, 'sql$|.csv$|.txt$', true, true );
		$count = count($files);
		$sql = array();
	for ($x=0; $x< $count; $x++ ) {
...
function export_files ()
	...
	$filesDir 	= 'components'.DIRECTORY_SEPARATOR.$option.DIRECTORY_SEPARATOR.'upload';
	$files 		= JFolder::files( $filesDir, 'clm$', true, true );
		$count = count($files);
		$sql = array();
	for ($x=0; $x< $count; $x++ ) {
		$sql[] = JFile::getName($files[$x]);
...
function liga_export ()
	...
	$now	= $date->toMySQL();
	$datum	= JHTML::_('date',  $now, JText::_('%d_%m_%Y__%H_%M'));
	...
	$path	= JPath::clean(JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.$option.DIRECTORY_SEPARATOR.'upload');
	$write	= $path.DIRECTORY_SEPARATOR.$file;
	...
	$sql = " SELECT name FROM #__clm_user "
		." WHERE sid = ".$liga_name[0]->sid
		." AND jid =".$jid
		;
	$db->setQuery( $sql );
	...
	$Dir = JPATH_ADMINISTRATOR .DIRECTORY_SEPARATOR. 'components'.DIRECTORY_SEPARATOR.'com_clm';
	$data = JApplicationHelper::parseXMLInstallFile($Dir.DIRECTORY_SEPARATOR.'clm.xml');
	...
	// Ligadaten
	if ($clm_cuser != "1") { $liga_name[0]->sl = JID; }
	if ( $clm_sql=="1") {
	...
	$ligadaten = $ligadaten."\n('".$liga_name[0]->id."','".utf8_decode($liga_name[0]->name)."','".$liga_name[0]->sid."','".$liga_name[0]->teil."','"
		.$liga_name[0]->stamm."','".$liga_name[0]->ersatz."','".$liga_name[0]->rang."','".JID."','"
		.$liga_name[0]->runden."','".$liga_name[0]->durchgang."','".$liga_name[0]->heim."','"
		.$liga_name[0]->mail."','".$liga_name[0]->sl_mail."','".$liga_name[0]->order."','"
		.$liga_name[0]->rnd."','".$liga_name[0]->published."','";
		...
		if ($clm_cuser != "1") { $liga_name[0]->sl = JID; }
	$ligadaten = "#1#"
		."\n('".utf8_decode($liga_name[0]->name)."','".$liga_name[0]->sid."','".$liga_name[0]->teil."','"
		.$liga_name[0]->stamm."','".$liga_name[0]->ersatz."','".$liga_name[0]->rang."','"
		.$liga_name[0]->sl."','".$liga_name[0]->runden."','".$liga_name[0]->durchgang."','"
		.$liga_name[0]->heim."','".$liga_name[0]->mail."','".$liga_name[0]->sl_mail."','"
		.$liga_name[0]->order."','".$liga_name[0]->rnd."','".$liga_name[0]->published."','";
			$neu = str_replace($replace,' ',$liga_name[0]->bemerkungen);
			$neu = str_replace($_surch,'\r\n',$neu);
		$ligadaten = $ligadaten.utf8_decode($neu)."'";
			$neu = str_replace($replace,' ',$liga_name[0]->bem_int);
	...
	// Mannschaftsdaten
	$sql = " SELECT * FROM #__clm_mannschaften "
		." WHERE liga = ".$liga
		." ORDER BY tln_nr "
		;
	$db->setQuery( $sql );
	...
	$end =",";
		for ($x=0; $x < count($man); $x++) {
			if ($x+1 == count($man)) { $end = ";";}
			if ($clm_cuser != "1") { $man[$x]->mf = JID; }
		$mannschaft = $mannschaft."\n('".$man[$x]->sid."','".utf8_decode($man[$x]->name)."','".$man[$x]->liga."','"
			.$man[$x]->zps."','".$man[$x]->liste."','".JID."','".$man[$x]->man_nr."','"
			.$man[$x]->tln_nr."','".JID."','".$man[$x]->datum."','".$man[$x]->edit_datum."'";
	...
	$mannschaft = "\n\n#".count($man)."#";
		for ($x=0; $x < count($man); $x++) {
			if ($clm_cuser != "1") { $man[$x]->mf = JID; }
		$mannschaft = $mannschaft."\n('".$man[$x]->sid."','".utf8_decode($man[$x]->name)."','".$man[$x]->liga."','"
			.$man[$x]->zps."','".$man[$x]->liste."','".$man[$x]->edit_liste."','".$man[$x]->man_nr."','"
			.$man[$x]->tln_nr."','".$man[$x]->mf."','".$man[$x]->datum."','".$man[$x]->edit_datum."'";
				$neu = str_replace($replace,' ',$man[$x]->lokal);
				$neu = str_replace($_surch,'\r\n',$neu);
					$mannschaft = $mannschaft.",'".utf8_decode($neu)."'";
				$neu = str_replace($replace,' ',$man[$x]->bemerkungen);
	...
	// DWZ Daten Spieler
	$sql = " SELECT s.* FROM #__clm_meldeliste_spieler as a"
		." LEFT JOIN #__clm_dwz_spieler AS s ON s.ZPS = a.zps AND s.Mgl_Nr = a.mgl_nr AND s.sid = a.sid "
		." WHERE a.lid = ".$liga
		." AND a.mgl_nr > 0 "
		." AND s.zps IS NOT NULL "
		." ORDER BY a.zps ASC , a.snr ASC "
		;
	$db->setQuery( $sql );
	...
	$end =",";
		for ($x=0; $x < count($dwz_spl); $x++) {
			if ($x+1 == count($dwz_spl)) { $end = ";";}
		$spl_dwz = $spl_dwz."\n('".$dwz_spl[$x]->sid."','".$dwz_spl[$x]->PKZ."','".$dwz_spl[$x]->ZPS."','".$dwz_spl[$x]->Mgl_Nr."','"
			.$dwz_spl[$x]->Status."','".utf8_decode($dwz_spl[$x]->Spielername)."','".utf8_decode($dwz_spl[$x]->Spielername_G)."','"
			.$dwz_spl[$x]->Geschlecht."','".$dwz_spl[$x]->Spielberechtigung."','".$dwz_spl[$x]->Geburtsjahr."','"
			.$dwz_spl[$x]->Letzte_Auswertung."','".$dwz_spl[$x]->DWZ."','".$dwz_spl[$x]->DWZ_Index."','"
			.$dwz_spl[$x]->FIDE_Elo."','".$dwz_spl[$x]->FIDE_Titel."','".$dwz_spl[$x]->FIDE_ID."','".$dwz_spl[$x]->FIDE_Land."')".$end
			;
		}}
	else {
	$spl_dwz = "\n\n#".count($dwz_spl)."#";
		...
		$spl_dwz = $spl_dwz."\n('".$dwz_spl[$x]->sid."','".$dwz_spl[$x]->PKZ."','".$dwz_spl[$x]->ZPS."','".$dwz_spl[$x]->Mgl_Nr."','"
			...
			.$dwz_spl[$x]->Geschlecht."','".$dwz_spl[$x]->Spielberechtigung."','".$dwz_spl[$x]->Geburtsjahr."','"
			.$dwz_spl[$x]->Letzte_Auswertung."','".$dwz_spl[$x]->DWZ."','".$dwz_spl[$x]->DWZ_Index."','"
			.$dwz_spl[$x]->FIDE_Elo."','".$dwz_spl[$x]->FIDE_Titel."','".$dwz_spl[$x]->FIDE_ID."','".$dwz_spl[$x]->FIDE_Land."');"
			;
		}}}

	// Meldelisten
	$sql = " SELECT * FROM #__clm_meldeliste_spieler "
		." WHERE lid = ".$liga
		." ORDER BY zps ASC , snr ASC "
		;
	$db->setQuery( $sql );
	...
	$end =",";
		for ($x=0; $x < count($ml); $x++) {
			if ($x+1 == count($ml)) { $end = ";";}
		$liste = $liste."\n('".$ml[$x]->sid."','".$ml[$x]->lid."','".$ml[$x]->mnr."','"
			.$ml[$x]->snr."','".$ml[$x]->mgl_nr."','".$ml[$x]->zps."','".$ml[$x]->status."','"
			.$ml[$x]->ordering."','".$ml[$x]->DWZ."','".$ml[$x]->I0."','".$ml[$x]->Punkte."','"
			.$ml[$x]->Partien."','".$ml[$x]->We."','".$ml[$x]->Leistung."','".$ml[$x]->EFaktor."','"
			.$ml[$x]->Niveau."','".$ml[$x]->sum_saison."')".$end
			;
		}}
	else {
	$liste = "\n\n#".count($ml)."#";
		for ($x=0; $x < count($ml); $x++) {
			$liste = $liste."\n('".$ml[$x]->sid."','".$ml[$x]->lid."','".$ml[$x]->mnr."','"
				...
				.$ml[$x]->ordering."','".$ml[$x]->DWZ."','".$ml[$x]->I0."','".$ml[$x]->Punkte."','"
				.$ml[$x]->Partien."','".$ml[$x]->We."','".$ml[$x]->Leistung."','".$ml[$x]->EFaktor."','"
				.$ml[$x]->Niveau."','".$ml[$x]->sum_saison."');"
				;
		}}}

	// Ranglisten
	// Rangliste ID
	$sql = " SELECT r.* FROM #__clm_liga as a "
		." LEFT JOIN #__clm_rangliste_id as r ON r.gid = a.rang AND r.sid = a.sid "
		." WHERE a.id = ".$liga
		." AND r.sid IS NOT NULL "
		;
	$db->setQuery( $sql );
	...
	$end =",";
		for ($x=0; $x < count($rang_id); $x++) {
			if ($x+1 == count($rang_id)) { $end = ";";}
		$id_rang = $id_rang."\n('".$rang_id[$x]->gid."','".$rang_id[$x]->sid."','".$rang_id[$x]->zps."','"
			.$rang_id[$x]->rang."','".$rang_id[$x]->published."','".$rang_id[$x]->bemerkungen."','".$rang_id[$x]->bem_int."','"
			.$rang_id[$x]->ordering."')".$end
			;
		}}
	if ( $clm_sql !="1") {
	$id_rang = "\n\n#".count($rang_id)."#";
	...
	// Rangliste Name
	$sql = " SELECT r.* FROM #__clm_liga as a "
		." LEFT JOIN #__clm_rangliste_name as r ON r.id = a.rang AND r.sid = a.sid "
		." WHERE a.id = ".$liga
		." AND r.sid IS NOT NULL "
		;
	$db->setQuery( $sql );
	...
	if ( $clm_sql=="1" AND count($rang_name) >0) {
		$name_rang = "\n\nINSERT INTO `#__clm_rangliste_name` (`Gruppe`, `Meldeschluss`, `geschlecht`, `alter_grenze`, `alter`, `sid`, `user`, `user_clm`, `bemerkungen`, `bem_int`, `ordering`, `published` ) VALUES ";
	$end =",";
		for ($x=0; $x < count($rang_name); $x++) {
			if ($x+1 == count($rang_name)) { $end = ";";}
		$name_rang = $name_rang."\n('".$rang_name[$x]->Gruppe."','".$rang_name[$x]->Meldeschluss."','".$rang_name[$x]->geschlecht."','"
			.$rang_name[$x]->alter_grenze."','".$rang_name[$x]->alter."','".$rang_name[$x]->sid."','".$rang_name[$x]->user."','"
			.$rang_name[$x]->user_clm."','".$rang_name[$x]->bemerkungen."','".$rang_name[$x]->bem_int."','"
			.$rang_name[$x]->ordering."','".$rang_name[$x]->published."')".$end
			;
		}}
	if ( $clm_sql !="1") {
	$name_rang = "\n\n#".count($rang_name)."#";
		...
		$name_rang = $name_rang."\n('".$rang_name[$x]->Gruppe."','".$rang_name[$x]->Meldeschluss."','".$rang_name[$x]->geschlecht."','"
			.$rang_name[$x]->alter_grenze."','".$rang_name[$x]->alter."','".$rang_name[$x]->sid."','".$rang_name[$x]->user."','"
			.$rang_name[$x]->user_clm."','".$rang_name[$x]->bemerkungen."','".$rang_name[$x]->bem_int."','"
			.$rang_name[$x]->ordering."','".$rang_name[$x]->published."');"
			;
		}}

	// Rangliste Spieler
	$sql = " SELECT r.* FROM #__clm_liga as a "
		." LEFT JOIN #__clm_rangliste_spieler as r ON r.Gruppe = a.rang AND r.sid = a.sid "
		." WHERE a.id = ".$liga
		." AND r.sid IS NOT NULL "
		;
	$db->setQuery( $sql );
	...
	$end =",";
		for ($x=0; $x < count($rang_spl); $x++) {
			if ($x+1 == count($rang_spl)) { $end = ";";}
		$spl_rang = $spl_rang."\n('".$rang_spl[$x]->Gruppe."','".$rang_spl[$x]->ZPS."','".$rang_spl[$x]->Mgl_Nr."','"
			.$rang_spl[$x]->PKZ."','".$rang_spl[$x]->Rang."','".$rang_spl[$x]->man_nr."','".$rang_spl[$x]->sid."')".$end
			;
		}}
	if ( $clm_sql !="1") {
	$spl_rang = "\n\n#".count($rang_spl)."#";
		for ($x=0; $x < count($rang_spl); $x++) {
		$spl_rang = $spl_rang."\n('".$rang_spl[$x]->Gruppe."','".$rang_spl[$x]->ZPS."','".$rang_spl[$x]->Mgl_Nr."','"
			.$rang_spl[$x]->PKZ."','".$rang_spl[$x]->Rang."','".$rang_spl[$x]->man_nr."','".$rang_spl[$x]->sid."');"
			;
		}}

	// Mannschaftsrunden
	$sql = " SELECT * FROM #__clm_rnd_man "
		." WHERE lid = ".$liga
		." ORDER BY dg ASC, runde ASC , paar ASC, heim ASC "
		;
	$db->setQuery( $sql );
	...
	$end =",";
		for ($x=0; $x < count($mrnd); $x++) {
			if ($x+1 == count($mrnd)) { $end = ";";}
			if ( $mrnd[$x]->brettpunkte =="") { $bp = "NULL"; }
				else { $bp = $mrnd[$x]->brettpunkte; }
			if ( $mrnd[$x]->manpunkte =="") { $mp = "NULL"; }
				else { $mp = $mrnd[$x]->manpunkte; }
			if ( $mrnd[$x]->bp_sum =="") { $bp_sum = "NULL"; }
				else { $bp_sum = $mrnd[$x]->bp_sum; }
			if ( $mrnd[$x]->mp_sum =="") { $mp_sum = "NULL"; }
				else { $mp_sum = $mrnd[$x]->mp_sum; }
			if ( $mrnd[$x]->gemeldet =="") { $gemeldet = "NULL"; }
				else { $gemeldet = JID; }
			if ( $mrnd[$x]->editor =="") { $editor = "NULL"; }
				else { $editor = JID; }
			if ( $mrnd[$x]->dwz_editor =="") { $dwz_editor = "NULL"; }
				else { $dwz_editor = JID; }
		$man_rnd = $man_rnd."\n('".$mrnd[$x]->sid."','".$mrnd[$x]->lid."','".$mrnd[$x]->runde."','"
			.$mrnd[$x]->paar."','".$mrnd[$x]->dg."','".$mrnd[$x]->heim."','".$mrnd[$x]->tln_nr."','"
			.$mrnd[$x]->gegner."','".$bp."','".$mp."','".$bp_sum."','".$mp_sum."','"
			.$gemeldet."','".$editor."','".$dwz_editor."','".$mrnd[$x]->zeit."','"
			.$mrnd[$x]->edit_zeit."','".$mrnd[$x]->dwz_zeit."','".$mrnd[$x]->published."','"
			.$mrnd[$x]->ordering."')".$end
			;
			}}
	else {
	$man_rnd = "\n\n#".count($mrnd)."#";
		for ($x=0; $x < count($mrnd); $x++) {
			...
			if ( $mrnd[$x]->gemeldet =="") { $gemeldet = "NULL"; }
					...
					if ($clm_cuser != "1") { $gemeldet = JID; }
					else { $gemeldet = $mrnd[$x]->gemeldet; }
					}
			if ( $mrnd[$x]->editor =="") { $editor = "NULL"; }
				else {
					if ($clm_cuser != "1") { $editor = JID; }
					else { $editor = $mrnd[$x]->editor; }
					}
	
			if ( $mrnd[$x]->dwz_editor =="") { $dwz_editor = "NULL"; }
				else {
					if ($clm_cuser != "1") { $dwz_editor = JID; }
					else { $dwz_editor = $mrnd[$x]->dwz_editor; }
					}
		$man_rnd = $man_rnd."\n('".$mrnd[$x]->sid."','".$mrnd[$x]->lid."','".$mrnd[$x]->runde."','"
			.$mrnd[$x]->paar."','".$mrnd[$x]->dg."','".$mrnd[$x]->heim."','".$mrnd[$x]->tln_nr."','"
			.$mrnd[$x]->gegner."','".$bp."','".$mp."','".$bp_sum."','".$mp_sum."','"
	...
	$end =",";
		for ($x=0; $x < count($srnd); $x++) {
			if ($x+1 == count($srnd)) { $end = ";";}
			if ( $srnd[$x]->dwz_edit =="") { $dwz_edit = "NULL"; }
				else { $dwz_edit = $srnd[$x]->dwz_edit; }
			if ( $srnd[$x]->dwz_editor =="") { $dwz_editor = "NULL"; }
				else { $dwz_editor = JID; }
			if ( $srnd[$x]->gemeldet == "") { $gemeldet ="NULL"; }
				else { $gemeldet = JID; }
		$spl_rnd = $spl_rnd."\n('".$srnd[$x]->sid."','".$srnd[$x]->lid."','".$srnd[$x]->runde."','"
			.$srnd[$x]->paar."','".$srnd[$x]->dg."','".$srnd[$x]->tln_nr."','"
			.$srnd[$x]->brett."','".$srnd[$x]->heim."','".$srnd[$x]->weiss."','"
			.$srnd[$x]->spieler."','".$srnd[$x]->zps."','".$srnd[$x]->gegner."','"
			.$srnd[$x]->gzps."','".$srnd[$x]->ergebnis."','".$srnd[$x]->kampflos."','"
			.$srnd[$x]->punkte."','".$gemeldet."','".$dwz_edit."','"
			.$dwz_editor."')".$end
			;
		}}
	else {
	$spl_rnd = "\n\n#".count($srnd)."#";
		for ($x=0; $x < count($srnd); $x++) {
			...
			if ($srnd[$x]->dwz_editor =="") { $dwz_editor = "NULL"; }
					...
					if ($clm_cuser != "1") { $dwz_editor = JID; }
					else { $dwz_editor = $srnd[$x]->dwz_editor; }
					}
			if ($srnd[$x]->gemeldet == "") { $gemeldet ="NULL"; }
				else {
					if ($clm_cuser != "1") { $gemeldet = JID; }
					else { $gemeldet = $srnd[$x]->gemeldet; }
					}
		$spl_rnd = $spl_rnd."\n('".$srnd[$x]->sid."','".$srnd[$x]->lid."','".$srnd[$x]->runde."','"
			.$srnd[$x]->paar."','".$srnd[$x]->dg."','".$srnd[$x]->tln_nr."','"
			.$srnd[$x]->brett."','".$srnd[$x]->heim."','".$srnd[$x]->weiss."','"
	...
	$end =",";
		for ($x=0; $x < count($trnd); $x++) {
			if ($x+1 == count($trnd)) { $end = ";";}
			if ($trnd[$x]->gemeldet =="") { $gemeldet = "NULL"; }
				else { $gemeldet = JID; }
			if ($trnd[$x]->editor =="") { $editor = "NULL"; }
				else { $editor = JID; }
		$ter_rnd = $ter_rnd."\n('".$trnd[$x]->sid."','".$trnd[$x]->name."','".$trnd[$x]->liga."','"
			.$trnd[$x]->nr."','".$trnd[$x]->datum."','".$trnd[$x]->meldung."','"
			.$trnd[$x]->sl_ok."','".$trnd[$x]->published."'";
			...
			$ter_rnd = $ter_rnd.",'".utf8_decode($neu)."'";
				...
				$neu = str_replace($_surch,'\r\n',$neu);
			$ter_rnd = $ter_rnd.",'".utf8_decode($neu)."','"
			.$gemeldet."','".$editor."','".$trnd[$x]->zeit."','"
			.$trnd[$x]->edit_zeit."','".$trnd[$x]->ordering."')".$end
			;
		}}
	else {
	$ter_rnd = "\n\n#".count($trnd)."#";
		for ($x=0; $x < count($trnd); $x++) {
			if ($trnd[$x]->gemeldet =="") { $gemeldet = "NULL"; }
					...
					if ($clm_cuser != "1") { $gemeldet = JID; }
					else { $gemeldet = $trnd[$x]->gemeldet; }
					}
			if ($trnd[$x]->editor =="") { $editor = "NULL"; }
				else {
					if ($clm_cuser != "1") { $editor = JID; }
					else { $editor = $trnd[$x]->gemeldet; }
					}
			$ter_rnd = $ter_rnd."\n('".$trnd[$x]->sid."','".$trnd[$x]->name."','".$trnd[$x]->liga."','"
				.$trnd[$x]->nr."','".$trnd[$x]->datum."','".$trnd[$x]->meldung."','"
				.$trnd[$x]->sl_ok."','".$trnd[$x]->published."'";
				...
				$ter_rnd = $ter_rnd.",'".utf8_decode($neu)."','"
				.$gemeldet."','".$editor."','".$trnd[$x]->zeit."','"
				.$trnd[$x]->edit_zeit."','".$trnd[$x]->ordering."');"
					;
		}}}

	// CLM User
	if(($clm_cuser =="1" OR $clm_juser =="1") AND $clm_sql !="1") {
	$sql = " SELECT u.jid FROM #__clm_mannschaften as a "
		." LEFT JOIN #__clm_user as u ON u.jid = a.mf AND u.sid = a.sid"
		." WHERE a.liga = $liga AND a.mf > 0 "
		." AND a.sid = ".$liga_name[0]->sid
	...
	for ($x=0; $x < count($clm); $x++) {
		if ($clm[$x]->jid != "") {
			$juser_jid[$clm[$x]->jid] = $clm[$x]->jid;
				if ($liga_name[0]->sl == $clm[$x]->jid ) { $sl = "1";}
			}
		}
	if($sl !="1") {
		$sql = " SELECT jid FROM #__clm_user "
			." WHERE jid = ".$liga_name[0]->sl
			." AND sid = ".$liga_name[0]->sid
			." AND jid IS NOT NULL "
			;
		$db->setQuery( $sql );
		$clm = $db->loadObjectList();
			if ($clm[0]->jid != "") { $juser_jid[$clm[0]->jid] = $clm[0]->jid;}
		}
	$jids = implode( ',', $juser_jid );

	$sql = " SELECT * FROM #__clm_user "
		." WHERE sid = ".$liga_name[0]->sid
		." AND jid IN ($jids)"
		;
	$db->setQuery( $sql );
	$clm = $db->loadObjectList();
		...
		$clm_user = $clm_user."\n('".$clm[$x]->sid."','".$clm[$x]->jid."','"
			.utf8_decode($clm[$x]->name)."','".utf8_decode($clm[$x]->username)."','"
			.$clm[$x]->aktive."','".utf8_decode($clm[$x]->email)."','".$clm[$x]->usertype."','"
			.utf8_decode($clm[$x]->tel_fest)."','".utf8_decode($clm[$x]->tel_mobil)."','"
			.$clm[$x]->user_clm."','".$clm[$x]->zps."','".$clm[$x]->published."'";
	...
	$clm = $db->loadObjectList();
		...
		for ($x=0; $x < count($clm); $x++) {
		$juser = $juser."\n('".utf8_decode($clm[$x]->name."','".$clm[$x]->username."','".$clm[$x]->email."','"
			.$clm[$x]->password."','".$clm[$x]->usertype."','".$clm[$x]->block."','"
			.$clm[$x]->sendEmail."','".$clm[$x]->gid."','".$clm[$x]->registerDate."','"
			.$clm[$x]->lastvisitDate."','".$clm[$x]->activation."','");
// evtl. Standardparameter verwenden -> Sicherheit !!
	...
	$buffer = 	$comment.$ligadaten.$dwz_verein.$mannschaft.$spl_dwz.$liste.$name_rang.$id_rang.$spl_rang.
			$man_rnd.$spl_rnd.$ter_rnd.$clm_user.$juser;
	...
	if ( $clm_sql=="1") { $msg = JText::_( 'DB_SQL'); }
	else { $msg = JText::_( 'DB_CLM'); }
	//$mainframe->redirect( 'index.php?option='. $option.'&section='.$section , $msg);
	$this->setRedirect('index.php?option=com_clm&view=db',$msg); 
	}
...
function saison ()
	...
	$sql = " SELECT id,name FROM #__clm_saison "
		." WHERE archiv = 0 ORDER BY id ASC "
		;
	$db->setQuery( $sql );
	$sid = $db->loadObjectList();
...
function sql_db ()
	...
	$section	= JRequest::getVar('section');
	$datei		= JRequest::getVar('sql_datei');
	$sql_del	= JRequest::getVar('sql_del');
	...
	$config	= &JComponentHelper::getParams( 'com_clm' );
	$version= $config->get('version',0);
	// check, ob execute_sql erlaubt
	if ($config->get('execute_sql', 0) != 1) {
		JError::raiseWarning( 500, JText::_( 'DB_SQLEXECUTE_0' ) );
		//$mainframe->redirect( 'index.php?option='. $option.'&section='.$section );
		$this->setRedirect('index.php?option=com_clm&view=db'); 
	}

	$filesDir 	= 'components'.DIRECTORY_SEPARATOR.$option.DIRECTORY_SEPARATOR.'upload'.DIRECTORY_SEPARATOR.$datei;
	

	// Deutsche oder Niederländische Version 
	if($version == 0 or $version == 1) {
		
		//Löschen der Übernahme-Tabellen vor neuer Übernahme
		if ($version == 0 AND $sql_del == "1") {
			if ($datei == "spieler.sql") {
				$sql = " DELETE FROM `dwz_spieler` ";
				$db->setQuery($sql);
				$db->query();
			}
			if ($datei == "vereine.sql") {
				$sql = " DELETE FROM `dwz_vereine` ";
				$db->setQuery($sql);
				$db->query();
			}
			if ($datei == "verband.sql") {
				$sql = " DELETE FROM `dwz_verbaende` ";
				$db->setQuery($sql);
				$db->query();
			}
		}
		...
		if ($handle = fopen($filesDir, "r")) {
			...
			while ($zeile = fgets($handle)) {
				$db->setQuery(utf8_encode($zeile));
				$db->query();
				// JError::raiseNotice( 500, utf8_encode($zeile) );
				$datCounter++;
			}

			// JError::raiseNotice( 500, memory_get_usage() );
			...
			JError::raiseWarning( 500, JText::_( 'DB_LESEN') );
			JError::raiseNotice( 6000,  JText::_( 'DB_RECHTE' ));
			//$mainframe->redirect( 'index.php?option='. $option.'&section='.$section );
			$this->setRedirect('index.php?option=com_clm&view=db'); 
		}
	...
	//JError::raiseNotice( 6000,  JText::_( 'Englische Version !' ).'-->'.$dat.'-->'.$Nr);
	//$mainframe->redirect( 'index.php?option='. $option.'&section='.$section );

	}
	

	// Message ausgeben - unabhängig vom Logfile-Eintrag
	if ($version == 0 or $version == 1) {
		$dat = $datCounter; // count($teil);
		$msg = JText::_( 'DB_AUS').' '.$datei.',  '.$dat.' '.JText::_('DB_DATEN');
	} elseif ($version == 2) {
		$dat = $counter;
		$msg = JText::_( 'DB_AUS').' '.$datei.',  '.$dat.' '.JText::_('DB_DATEN');
	}
	...
	// Log schreiben
	$clmLog = new CLMLog();
	$clmLog->aktion = JText::_('DB_BUTTON_SQL_EXECUTE').": ".$datei;
	$clmLog->params = array('cids' => $dat);
	$clmLog->write();
...
function import ()
	...
	if($pub_user =="1") { $publish = 0; $block =1; } else { $publish = 1; $block =0; }
	...
	jimport('joomla.filesystem.file');
	$path 	= JPath::clean(JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.$option.DIRECTORY_SEPARATOR.'upload');
	$content= JFile::read($path.DIRECTORY_SEPARATOR.$datei);
	...
	$fehler = 0;
	if($cnt_man+1 > count($man_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_KLEIN')); $fehler++;}
	if($cnt_man+1 < count($man_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_GROSS')); $fehler++;}
	if($cnt_man != $liga_daten[2] OR count($man_daten)-1 != $liga_daten[2] )
		{ JError::raiseWarning( 500, JText::_('DB_ANZAHL_FALSCH')); $fehler++;}

	if($cnt_spl+1 > count($melde_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_SP_KLEIN')); $fehler++;}
	if($cnt_spl+1 < count($melde_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_SP_GROSS')); $fehler++;}

	if($cnt_rnd_man+1 > count($man_rnd_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_MR_KLEIN')); $fehler++;}
	if($cnt_rnd_man+1 < count($man_rnd_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_MR_GROSS')); $fehler++;}
	if(($cnt_rnd_man != ($liga_daten[2]*$liga_daten[7]*$liga_daten[8])) OR (count($man_rnd_daten)-1 != ($liga_daten[2]*$liga_daten[7]*$liga_daten[8])))
		{ JError::raiseWarning( 500, JText::_('DB_ANZAHL_MR_FALSCH')); $fehler++;}

	if($cnt_rnd_spl+1 > count($spl_rnd_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_SR_KLEIN')); $fehler++;}
	if($cnt_rnd_spl+1 < count($spl_rnd_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_SR_GROSS')); $fehler++;}

	if($cnt_rnd_ter+1 > count($rnd_ter_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_RU_KLEIN')); $fehler++;}
	if($cnt_rnd_ter+1 < count($rnd_ter_daten)) { JError::raiseWarning( 500, JText::_('DB_ANZAHL_RU_GROSS')); $fehler++;}

	if($cnt_user > (1+$liga_daten[2])) { JError::raiseWarning( 500, JText::_('DB_CLM_NUTZER')); $fehler++;}

	if($cnt_juser > (1+$liga_daten[2])) { JError::raiseWarning( 500, JText::_('DB_JOOMLA_NUTZER')); $fehler++;}
	...
	for($x=0; $x < $cnt_user; $x++) {
		...
		else {	$email_clmid[$email]	= $clm_exist[0]->id; }
	...
	for($x=0; $x < $cnt_juser; $x++) {
		...
		else { $email_josid[$email] = $jos_exist[0]->id; }
		}
	}
	...
	$sql = " SELECT ZPS FROM #__clm_dwz_vereine "
		." WHERE sid = ".$saison
		." AND ZPS ='$zps'"
		;
	$db->setQuery($sql);
	...
	if($import > 0) { $row->mf = $import; }
	else { $row->mf		= $jid_new[(int)$man_einzel[8]]; }
	...
	$sql = " SELECT ZPS FROM #__clm_dwz_spieler "
		." WHERE sid = ".$saison
		." AND ZPS ='$zps'"
		." AND Mgl_Nr =".$mgl
		;
	...
	$f_land	= utf8_encode(substr($spieler[16], 0, -2));
		...
		$sql = " INSERT INTO `#__clm_dwz_spieler` ( `sid`, `PKZ`, `ZPS`, `Mgl_Nr`, `Status`, `Spielername`, `Spielername_G`, `Geschlecht`, `Spielberechtigung`, `Geburtsjahr`, `Letzte_Auswertung`, `DWZ`, `DWZ_Index`, `FIDE_Elo`, `FIDE_Titel`, `FIDE_ID`, `FIDE_Land`) VALUES "
		." ('$saison','$pkz','$zps','$mgl','$status','$name','$name_g','$gesch','$berech','$geb_j','$ausw','$dwz','$dwz_i','$elo','$titel','$f_id','$f_land')"
			;
		$db->setQuery($sql);
		$db->query();
		}
	...
	for ($x=0; $x< $cnt_spl; $x++) {
		$spl_einzel	= explode ("','", $melde_daten[$x]);
			...
			if (!$row->store()) { return JError::raiseWarning( 500,$row->getError() );}
			*/
	if ($liga =="new") {	$mlid = $lid; }
		else {		$mlid = $liga; }
	$sum_saison = substr($spl_einzel[16], 0, -2);
	$query	= "INSERT INTO #__clm_meldeliste_spieler "
		." ( `sid`, `lid`, `mnr`, `snr`, `mgl_nr`, `zps`, `status`, `ordering`, `DWZ`, `I0`, `Punkte`, `Partien`, `We`, `Leistung`, `EFaktor`, `Niveau`, `sum_saison`) "
		." VALUES ('$saison','$mlid','$spl_einzel[2]','$spl_einzel[3]','$spl_einzel[4]','$spl_einzel[5]','$spl_einzel[6]','$spl_einzel[7]','$spl_einzel[8]','$spl_einzel[9]','$spl_einzel[10]','$spl_einzel[11]','$spl_einzel[12]','$spl_einzel[13]','$spl_einzel[14]','$spl_einzel[15]','$sum_saison') "
		;
	$db->setQuery($query);
	$db->query();
	}
	...
	if($man_rnd[12] !="NULL" AND $jid_new[$man_rnd[12]] != "")
		...
		if($import > 1) { $row->gemeldet = $import; }
		}
		if($man_rnd[12] !="NULL" AND $jid_new[$man_rnd[12]] =="")
		{ $row->gemeldet	= $sl;
		if($import > 1) { $row->gemeldet = $import; }
	...
	if($man_rnd[14] !="NULL" AND $jid_new[$man_rnd[14]] != "")
		...
		if($import > 1) { $row->dwz_editor = $import; }
		}
		if($man_rnd[14] !="NULL" AND $jid_new[$man_rnd[14]] =="")
		{ $row->dwz_editor	= $sl;
		if($import > 1) { $row->dwz_editor = $import; }
	...
	$query	=" SELECT id FROM #__clm_rangliste_name "
		." WHERE Gruppe = '".$gruppe."'"
		." AND Meldeschluss = '".$melde."'"
		." AND geschlecht  = '".$gesch."'"
		." AND alter_grenze = ".$grenze
		." AND `alter` = ".$alter
		." AND sid = ".$saison
		." AND user = ".$ruser
		." AND user_clm = ".$r_clm
	...
	$query	=" SELECT id FROM #__clm_rangliste_id "
		." WHERE gid = '".$res."'"
		." AND sid = ".$saison
		." AND zps = '$zps'"
		;
	...
	$query	=" SELECT Rang FROM #__clm_rangliste_spieler "
		." WHERE Gruppe = '".$res."'"
		." AND ZPS = '$saison'"
		." AND Mgl_Nr = '$zps'"
		;
...
function convert_db ()
	...
	$path		= 'components'.DIRECTORY_SEPARATOR.$option.DIRECTORY_SEPARATOR.'upload'.DIRECTORY_SEPARATOR;
...
////////////////////////////////////////
// Diese Funktionen sind außer Dienst //
// evtl. Verwendung als Vorlage ...   //
////////////////////////////////////////
...
function upload_php()
	...
	$file		= JRequest::getVar( 'datei');
	$file = $_FILES['datei']['tmp_name'];
	move_uploaded_file($file, "datei.txt");
	$msg = JText::_( 'DB_UPLOAD_START').' '.$file;
	//$mainframe->redirect( 'index.php?option='. $option.'&section='.$section , $msg);
	$this->setRedirect('index.php?option=com_clm&view=db',$msg); 
	}
...
function upload()
	...
	$src = '/zugriff.inc';
	$dest = 'hp.DIRECTORY_SEPARATOR.www.DIRECTORY_SEPARATOR.fishpoke.DIRECTORY_SEPARATOR.administrator.DIRECTORY_SEPARATOR.clock_copy.jpg';
	$path = 'hp.DIRECTORY_SEPARATOR.Www.DIRECTORY_SEPARATOR.fishpoke.DIRECTORY_SEPARATOR.TEST';

	$path = '/administrator/POKE/';
	$path = JPath::clean(str_replace($path), '/');
...
function copy($src, $dest, $path = null)
    {
        ...
        // Basispfad
        if ($path) {
            $src = JPath::clean($path.DIRECTORY_SEPARATOR.$src);
            $dest = JPath::clean($path.DIRECTORY_SEPARATOR.$dest);
        }
        // Quellpfad testen
        if (!is_readable($src)) {
	...
	//weitere DB-Funktionen
	function othersDB() {	
		
		$this->setRedirect( 'index.php?option=com_clm&view=dewis' );
