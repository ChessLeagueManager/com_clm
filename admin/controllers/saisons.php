...
class CLMControllerSaisons extends JControllerLegacy {
	...
	function display($cachable = false, $urlparams = array()) {
		...
		if ($counter > 1) {
			...
			$this->setMessage(Text::_('SAISON_GIBT') . ' ' . $counter . ' ' . Text::_('SAISON_AKTIVE') . ' ' . ($counter - 1) . ' ' . Text::_('SAISON') . ' ' . $s . ' ' . Text::_('SAISON_ZURUECK'), 'notice');		
		}
		// state filter
	...
	function edit() {
		...
		// Archiv
		$lists['archiv'] = HTMLHelper::_('select.booleanlist', 'archiv', 'class="inputbox"', $row->archiv);
		$lists['published'] = HTMLHelper::_('select.booleanlist', 'published', 'class="inputbox"', $row->published);
		$lists['rating_type'] = HTMLHelper::_('select.booleanlist', 'rating_type', 'class="inputbox"', $row->rating_type);
		require_once (JPATH_COMPONENT . DS . 'views' . DS . 'saisons.php');
	...
	function save() {
		...
		// Log schreiben
		$clmLog = new CLMLog();
		$clmLog->aktion = Text::_('SAISON_SAISON_GESPEI');
	...
	function remove() {
		...
		foreach ($cids as $cid) {
			...
			if ($out[0]) {
				// Log schreiben
				$clmLog = new CLMLog();
				$clmLog->aktion = Text::_('SAISON_AKTION_SEASON_DEL') . " " . $task;
	...
	function publish() {
		...
		foreach ($cids as $cid) {
			...
			if ($out[0]) {
				// Log schreiben
				$clmLog = new CLMLog();
				$clmLog->aktion = Text::_('SAISON_AKTION_SEASON') . " " . $task;
				$clmLog->params = array('sid' => clm_core::$access->getSeason(), 'cids' => $cid);
				$clmLog->write();
			}
		}
	...
	function saveOrder() {
		...
		$this->setMessage(Text::_('CLM_NEW_ORDERING_SAVED'));
		$this->setRedirect('index.php?option=' . $option . '&section=' . $section);
	}
	function copy() {
		// Check for request forgeries
		...
		$cid = clm_core::$load->request_array_int('cid');
		$db = Factory::getDBO();
		$table = Table::getInstance('saisons', 'TableCLM');
		$user = Factory::getUser();
		$n = count($cid);
		if ($n > 0) {
			foreach ($cid as $id) {
			...
			$this->setMessage(Text::_('SAISON_NO_SELECT'), 'warning');
			return;
		}
		if ($n > 1) {
			$msg = Text::_('SAISON_MSG_ENTRYS_COPY');
		} else {
			$msg = Text::_('SAISON_MSG_ENTRY_COPY');
		}
		// Log schreiben
		$clmLog = new CLMLog();
		$clmLog->aktion = Text::_('SAISON_AKTION_SEASON_COPY');
...
function dwz_start()          
	...
	if ($cid[0] < 1 ) {
		$this->setMessage(Text::_('SAISON_NO_AUSWERTEN'), 'warning');
		$this->setRedirect('index.php?option=' . $option . '&section=' . $section);
		return;
	}
...
function dwz_del()
	...
	// keine Saison gewählt
	if ($cid[0] < 1 ) {
		$this->setMessage(Text::_('SAISON_NO_LOESCH'), 'warning');
		$this->setRedirect('index.php?option=' . $option . '&section=' . $section);
		return;
	}
	//Saison wird durch User im Screen bestimmt
	...
	// Log schreiben
	$clmLog = new CLMLog();
	$clmLog->aktion = Text::_( 'SAISON_LOG_DWZ_DEL');
	$clmLog->nr_aktion = 102;	
	$clmLog->params = array('sid' => $sid, 'lid' => 0);  
	$clmLog->write();

	$msg = Text::_( 'SAISON_DWZ_IST_LOESCH');
	$this->setMessage($msg);
	$this->setRedirect('index.php?option=' . $option . '&section=' . $section);
	}
}
