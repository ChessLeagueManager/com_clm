...
class CLMControllerErgebnisse extends JControllerLegacy
{
    ...
    public function display($cachable = false, $urlparams = array())
    {
        ...
        if ($val == 1 and $runde != "") {
            $mainframe->setUserState("$option.filter_runde", "$runde");
        }
        if ($dg  != "") {
            $mainframe->setUserState("$option.filter_dg", "$dg");
        }
        ...
        if ($filter_runde and !$filter_lid) {
            $where[] = 'a.runde = '.(int) $filter_runde;
        }
        if ($filter_dg) {
        ...
        if ($search) {
            $where[] = 'LOWER(m.name) LIKE "'.$db->escape('%'.$search.'%').'"';
        }
        ...
        if (isset($rows[0]->liga) and is_string($rows[0]->liga)) {
            ...
            } elseif ($clmAccess->access('BE_'.$mppoint.'_edit_result') === true) {
                $where_sl = '';
            } else {
    ...
    public function edit()
    {
        ...
        if ($runde[0]->rang != "0") {
            $sql = $sql.",r.Rang as snr,r.man_nr as rmnr";
        }
    ...
    public function save_wertung()
    {
        ...
        if ($countryversion == "en" and ($runden_modus == 4 or $runden_modus == 5)) {
            $limit = $stamm * 2;
        }
        //	else $limit = $stamm;
        else {
            $limit = $stamm * ($sieg + $antritt);
        }
        $err = 0;
        ...
        if ($sieg_bed == 1) {
            ...
            if ($hmpunkte == $gmpunkte and $hmpunkte > 0) {
                $hman_punkte = $man_remis;
                $gman_punkte = $man_remis;
            }
            if ($hmpunkte == $gmpunkte and $hmpunkte == 0) {
    ...
    public function delete_wertung()
    {
        $mainframe	= JFactory::getApplication();
    ...
    */
    public function calculateRanking($sid, $liga)
    {
