...
class CLMControllerTurForm extends JControllerLegacy {
	...
	function save() {
		...
		if ($result[0]) { // erfolgreich?
			...
			if ($result[1]) { // neues Turnier?
				$app->enqueueMessage( Text::_('TOURNAMENT_CREATED') );
			} else {
				$app->enqueueMessage( Text::_('TOURNAMENT_EDITED') );
			}
	...
	function _saveDo() {
		...
		if (!$row->id) {
			...
			$stringAktion = Text::_('TOURNAMENT_CREATED');
			// $where = "sid = " . (int) $row->sid; warum nur in Saison?
			$row->ordering = $row->getNextOrder(); // ( $where );
		} else {
			$neu = false;
			$stringAktion = Text::_('TOURNAMENT_EDITED');
		}
		...
		if (!$row->store()) {
			...
			if ($row->lokal > '') {
				...
				if(is_null($lokal_coord)){
					$app->enqueueMessage(Text::_('WARNING_ADDRESS_LOOKUP'), 'warning');
				}
			}
		}
