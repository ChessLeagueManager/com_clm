...
class CLMControllerUsers extends JControllerLegacy
{
    ...
    public function display($cachable = false, $urlparams = array())
    {
        ...
        $where[] = ' c.published = 1';
        if ($filter_usertype) {
            $where[] = "a.usertype = '$filter_usertype'";
        }
        if ($filter_sid) {
            $where[] = 'a.sid = '.$filter_sid;
        }
        if ($filter_vid) {
        ...
        if ($usertypestring != "") {
            $sql .= ' WHERE !(usertype IN ('.$usertypestring.' )) ';
        }
        $sql .=	' ORDER BY ordering ASC ';
        $db->setQuery($sql);
        $utlist = $db->loadObjectList();
        for ($i = 0; $i < count($utlist); $i++) {
            if ($utlist[$i]->kind == "CLM") {
                $utlist[$i]->name = JText::_('ACCESSGROUP_NAME_'.$utlist[$i]->usertype);
            }
        }
    ...
    public function edit()
    {
        ...
        if ($usertypestring != "") {
            $sql .= 'AND !(usertype IN ('.$usertypestring.' )) ';
        }
    ...
    */
    ...
    public function send()
    {
        ...
        for ($i = 0; $i < $n; $i++) {
            ...
            if ($rows[$i]->aktive == '0') {
                ...
                } else { //				$mail->sendMail($from, $fromname, $recipient, $subject_neu, $body, 0, null, $bcc);
            ...
            elseif ($rows[$i]->aktive == '1') {
                ...
                } else { //				$mail->sendMail($from,$fromname,$recipient,$subject_remind,$body,0,null,$bcc);
    ...
    //UserAccessGroups
    public function showaccessgroups()
    {
        $cid		= clm_core::$load->request_array_int('cid');
