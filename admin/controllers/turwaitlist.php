...
class CLMControllerTurWaitlist extends JControllerLegacy {
	...
	function add() {
		...
		$adminLink->more = array('id' => $id, 'list' => 'wait');
		$adminLink->makeURL();
	...
	function del_player() {
		...
		$output = clm_core::$api->db_tournament_waitlist_del($id,$cid);
	...
	function _removeDo() {
		...
		for ($i=0; $i<count($cid); $i++) {
			...
			$query = 'DELETE FROM #__clm_turniere_tlnr_wl'
				.' WHERE turnier = '.$turnier->id.' AND id = '. $cid[$i];
			clm_core::$db->query($query);
			$query='UPDATE #__clm_turniere_tlnr_wl  SET snr = (snr - 1) '	
	...
	function _sortBy($by) {
		...
		// Anzahl gemeldeter Spieler -> maximale Snr
		$query = "SELECT COUNT(id) FROM `#__clm_turniere_tlnr_wl`"
			." WHERE turnier =".$id
			;
		$db->setQuery($query);
		...
		$table	=Table::getInstance( 'turnier_waitlist', 'TableCLM' );
		// Snr umsortieren
		$snr = 0;
...
*/
	...
	function player_decode_copy() {	
		...
		$row =Table::getInstance( 'decode', 'TableCLM' );
		$i = 0;
		...
		$this->app->enqueueMessage( $i.' '.Text::_('DECODE_SEASON_COPIED') );
		$this->app->redirect( $adminLink->url );
	}
	...
	function export_csv() {
		...
		// Log schreiben
		$clmLog = new CLMLog();
		$clmLog->aktion = "Teilnehmer.csv"." ".Text::_('CLM_EXPORT');
	...
	function export_pdf() {
		...
		// Log schreiben
		$clmLog = new CLMLog();
		$clmLog->aktion = "Teilnehmer.pdf"." ".Text::_('CLM_EXPORT');
		$clmLog->params = array('file_name' => $file_name, 'id' => $lid,'format' => 'csv'); 
		$clmLog->write();
