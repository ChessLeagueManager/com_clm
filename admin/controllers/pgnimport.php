...
class CLMControllerPGNImport extends JControllerLegacy
{
    ...
    public function dbgame($game)
    {
        ...
        if (isset($game['pgnnr']) and is_numeric($game['pgnnr']) and $game['pgnnr'] > 0) {
            $pgnnr = $game['pgnnr'];
        } else {
            $pgnnr = 0;
        }
        if (isset($game['sid']) and is_numeric($game['sid']) and $game['sid'] > 0) {
            $sid = $game['sid'];
        } else {
            $sid = 0;
        }
        ...
        if ($error_text == '' and isset($tkz) and ($tkz == 't' or $tkz == 's') and isset($tid) and $tid > '0') {
            ...
            if ($runde > 0) {
                $query .= ' AND a.runde = '.$runde;
            }
            if ($brett > 0) {
                $query .= ' AND a.brett = '.$brett;
            }
        ...
        if ($test_button > 0) {
            echo "<br>query: ";
            var_dump($query);
            echo "<br>Mögl. Paarungen: ";
            var_dump($gameslist);
            echo "<br>Anz. Mögl. Paarungen: ";
            var_dump(count($gameslist));
        }
        ...
        if ($test_button > 0) {
            echo "<br>Mögl. Namen: ";
            var_dump($gameslist_name);
            echo "<br>Anz. Mögl. Namen: ";
            var_dump(count($gameslist_name));
            echo "<br>pgnnr: $pgnnr  Bisherige Fehler: ";
            var_dump($error_text);
        }
        ...
        if ($tkz == 't') {
            $return_arr['paar'] = $pgnnr_arr[0]->paar;
        } else {
            $return_arr['paar'] = 0;
        }
    ...
    public function getTablePGN()
    {
        ...
        if ($test_button > 0) {
            echo "<br><br>Turnier: ";
            var_dump($turnier);
            echo "<br><br>Start Offene gegen Tabelle <br> Turnier: ";
            var_dump($liga);
            echo "<br><br>Saison-ID: ";
            var_dump($sid);
        }
        ...
        if ($test_button > 0) {
            echo "<br><br>Select-Query: ";
            var_dump($query);
            echo "<br><br>Offene Notationen: ";
            var_dump($gameslist);
            echo "<br><br>Anzahl: ";
            var_dump($anz);
        }
    ...
    public function getDelPGN($all = false)
    {
    ...
    public function maintain_ntable()
    {
        $liga = clm_core::$load->request_string('liga', '');
        $pgn_file = clm_core::$load->request_string('pgn_file', '');
        $adminLink = new AdminLink();
