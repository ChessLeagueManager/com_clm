...
class CLMSWT {
	...
	function readString ($file, $offset, $length = 1) {
		$i = 0;
		$name = '';
	...
	function readName ($file, $offset, $length = 1) {
		return CLMSWT::readString ($file, $offset, $length);
	}

	function readInt ($file, $offset, $length = 1) {
		$value = 0;
		for ($i = 0; $i < $length; $i++) {
	...
	function getFormValue($name, $default = null, $type = 'none', $keys = null) {
		if($keys !== null) {
